...
class Hyperparameters:
    ...
    context_patches = int(os.environ.get("CONTEXT_PATCHES", 3))  # number of context patches before the target
...
class CurrentPatchEncoder(nn.Module):
    def __init__(self, model_dim: int, patch_size: int):
        ...
        # Multi-head attention pooling (matching target encoder design)
    ...
    def forward(self, patch_byte_emb: Tensor) -> Tensor:
        ...
        x = F.silu(self.gate_proj(x)) * self.in_proj(x)  # SwiGLU gating
        x = F.rms_norm(x, (x.size(-1),))
...
class TargetPatchEncoder(nn.Module):
    ...
    def __init__(self, model_dim: int, patch_size: int):
        ...
        # Intra-patch self-attention: lets all bytes attend to each other (non-local)
        self.intra_num_heads = 4
        self.intra_head_dim = model_dim // self.intra_num_heads
        self.intra_qkv = nn.Linear(model_dim, 3 * model_dim, bias=False)
        self.intra_out = nn.Linear(model_dim, model_dim, bias=False)
    ...
    def forward(self, patch_byte_emb: Tensor) -> Tensor:
        ...
        # Intra-patch self-attention: non-local byte interactions within each patch
        x_sa = F.rms_norm(x, (x.size(-1),))
        x_flat_sa = x_sa.reshape(B * P, S, D)
        qkv = self.intra_qkv(x_flat_sa).reshape(B * P, S, 3, self.intra_num_heads, self.intra_head_dim)
        q, k, v = qkv.unbind(dim=2)  # each (B*P, S, H, Dh)
        q = q.transpose(1, 2)  # (B*P, H, S, Dh)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
        scale = self.intra_head_dim ** -0.5
        attn = (q @ k.transpose(-2, -1)) * scale
        attn = F.softmax(attn, dim=-1)
        sa_out = (attn @ v).transpose(1, 2).reshape(B * P, S, D)
        x = x + self.intra_out(sa_out).reshape(B, P, S, D)  # residual
        # Normalize after conv+SA residuals for stable attention pooling inputs
        x = F.rms_norm(x, (x.size(-1),))
...
class Predictor(nn.Module):
    def __init__(self, model_dim: int):
        super().__init__()
        # Two residual SwiGLU blocks with 2x expansion for current→target mapping
        # Standard SwiGLU uses 2-4x hidden expansion; 1x was underpowered
    ...
    def forward(self, x: Tensor) -> Tensor:
        # Block 1: SwiGLU with 2x expansion
        residual = x
        x = F.rms_norm(x, (x.size(-1),))
        x = F.silu(self.gate_a(x)) * self.up_a(x)
        x = self.down_a(x) + residual
        # Block 2: SwiGLU with 2x expansion
        residual = x
        x = F.rms_norm(x, (x.size(-1),))
        x = F.silu(self.gate_b(x)) * self.up_b(x)
        x = self.down_b(x) + residual
        return F.rms_norm(x, (x.size(-1),))


class ContextAggregator(nn.Module):
    """Two-layer causal self-attention to aggregate multi-patch context."""

    def __init__(self, model_dim: int, max_ctx: int):
        super().__init__()
        self.pos_emb = nn.Parameter(torch.zeros(max_ctx, model_dim, dtype=torch.float32))
        nn.init.normal_(self.pos_emb, mean=0.0, std=0.01)
        # Layer 1
        self.qkv1 = nn.Linear(model_dim, 3 * model_dim, bias=False)
        self.out_proj1 = nn.Linear(model_dim, model_dim, bias=False)
        # Layer 2
        self.qkv2 = nn.Linear(model_dim, 3 * model_dim, bias=False)
        self.out_proj2 = nn.Linear(model_dim, model_dim, bias=False)
        self.num_heads = 4
    ...
    def _attn_block(self, x: Tensor, qkv_proj: nn.Linear, out_proj: nn.Linear) -> Tensor:
        B, S, D = x.shape
        h = F.rms_norm(x, (D,))
        qkv = qkv_proj(h).reshape(B, S, 3, self.num_heads, self.head_dim)
        ...
        return x + out_proj(out)

    def forward(self, x: Tensor) -> Tensor:
        # x: (B, num_patches, D)
        B, S, D = x.shape
        x = x + self.pos_emb[:S].unsqueeze(0).to(dtype=x.dtype)
        x = self._attn_block(x, self.qkv1, self.out_proj1)
        x = self._attn_block(x, self.qkv2, self.out_proj2)
        return x
...
class ProbeModel(nn.Module):
    def __init__(self, args: Hyperparameters):
        ...
        self.context_patches = args.context_patches
        ...
        self.context_agg = ContextAggregator(args.model_dim, args.context_patches)
    ...
    def encode_current(self, patches: Tensor) -> Tensor:
        # patches: (B, context_patches, patch_size)
        enc = self.online_encoder(self.tok_emb(patches))  # (B, context_patches, D)
        if enc.size(1) > 1:
            enc = self.context_agg(enc)  # causal self-attention over context patches
        # Take the last context position as the summary for prediction
        return enc[:, -1:, :]  # (B, 1, D)
    ...
    def loss_on_pairs(self, current_patches: Tensor, next_patches: Tensor) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        total = 0.5 * mse + 0.35 * infonce + 0.15 * online_infonce + self.jepa_var_weight * var_loss + self.jepa_cov_weight * cov_loss
        aux = {
            "mse": mse.detach(),
...
def make_patch_pairs(stream, batch_size: int, patch_size: int, device: torch.device, context_patches: int = 1) -> tuple[Tensor, Tensor]:
    total_patches = context_patches + 1  # context + 1 target
    chunk = stream.take(batch_size * patch_size * total_patches)
    patches = chunk.reshape(batch_size, total_patches, patch_size)
    current = patches[:, :context_patches, :].to(device=device, dtype=torch.long)  # (B, context_patches, patch_size)
    nxt = patches[:, -1, :].to(device=device, dtype=torch.long)  # (B, patch_size)
    return current, nxt.unsqueeze(1)
...
def evaluate_probe(model: ProbeModel, stream, args: Hyperparameters, device: torch.device) -> dict[str, float]:
    ...
    for _ in range(args.eval_batches):
        current_patches, next_patches = make_patch_pairs(stream, args.batch_size, args.patch_size, device, args.context_patches)
...
def main() -> None:
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": model.context_agg.parameters(), "lr": args.lr},
    ...
    for step in range(1, args.probe_steps + 1):
        ...
        current_patches, next_patches = make_patch_pairs(train_stream, args.batch_size, args.patch_size, device, args.context_patches)
