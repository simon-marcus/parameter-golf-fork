...
class Predictor(nn.Module):
    def __init__(self, model_dim: int):
        super().__init__()
        # Two residual SwiGLU blocks for current→target mapping
        self.gate_a = nn.Linear(model_dim, model_dim, bias=False)
        self.up_a = nn.Linear(model_dim, model_dim, bias=False)
        self.down_a = nn.Linear(model_dim, model_dim, bias=False)
        self.gate_b = nn.Linear(model_dim, model_dim, bias=False)
        self.up_b = nn.Linear(model_dim, model_dim, bias=False)
        self.down_b = nn.Linear(model_dim, model_dim, bias=False)
