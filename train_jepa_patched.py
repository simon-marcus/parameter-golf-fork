"""
Patch-first byte-level JEPA trainer.

This is the first serious merch-lane architecture after we established that the
old 1024-byte backbone is throughput-bound rather than parameter-starved.

Key idea:
- keep raw bytes as the only alphabet
- summarize fixed byte patches into patch tokens
- shift those patch tokens right, so the transformer only sees previous patches
- decode exact bytes inside each target patch with a tiny local autoregressive decoder
- apply JEPA to next-patch latents
"""
...
class Hyperparameters:
    ...
    patch_size = int(os.environ.get("PATCH_SIZE", 2))
    patch_embed_mode = os.environ.get("PATCH_EMBED_MODE", "flat").strip().lower()
...
class PatchReducer(nn.Module):
    def __init__(self, model_dim: int, patch_size: int, mode: str, num_heads: int):
        super().__init__()
        self.model_dim = model_dim
        self.patch_size = patch_size
        self.mode = mode
        if mode not in {"flat", "mixer", "attn"}:
            raise ValueError(f"PATCH_EMBED_MODE must be one of flat,mixer,attn; got {mode}")

        self.flat_proj = naivebaseline.CastedLinear(model_dim * patch_size, model_dim, bias=False)

        if mode in {"mixer", "attn"}:
            self.byte_pos_emb = nn.Parameter(torch.zeros(patch_size, model_dim, dtype=torch.float32))
        else:
            self.register_parameter("byte_pos_emb", None)
        ...
        if mode == "mixer":
            ...
            self.mix_in = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
            self.mix_out = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
            self.mix_out._zero_init = True
            self.mix_proj = naivebaseline.CastedLinear(model_dim * patch_size, model_dim, bias=False)
        else:
            self.dw_conv = None
            self.mix_in = None
            self.mix_out = None
        ...
        if mode == "attn":
            if model_dim % num_heads != 0:
                raise ValueError(f"model_dim={model_dim} must be divisible by num_heads={num_heads} for PATCH_EMBED_MODE=attn")
            self.num_heads = num_heads
            self.head_dim = model_dim // num_heads
            self.qkv = naivebaseline.CastedLinear(model_dim, 3 * model_dim, bias=False)
            self.attn_out = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
            self.attn_ff_in = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
            self.attn_ff_out = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
            self.attn_ff_out._zero_init = True
            self.register_buffer(
                "attn_mask",
                torch.triu(torch.ones(patch_size, patch_size, dtype=torch.bool), diagonal=1),
                persistent=False,
            )
        else:
            self.qkv = None
            self.attn_out = None
            self.attn_ff_in = None
            self.attn_ff_out = None
            self.num_heads = 0
            self.head_dim = 0
    ...
    def _init_weights(self) -> None:
        if self.byte_pos_emb is not None:
            nn.init.normal_(self.byte_pos_emb, mean=0.0, std=0.005)
        for module in self.modules():
            if isinstance(module, nn.Linear) and getattr(module, "_zero_init", False):
                nn.init.zeros_(module.weight)

    def forward(self, patch_emb: Tensor) -> Tensor:
        bsz, num_patches, patch_size, dim = patch_emb.shape
        flat = patch_emb.reshape(bsz, num_patches, patch_size * dim)
        if self.mode == "flat":
            return self.flat_proj(flat)

        x = patch_emb.reshape(bsz * num_patches, patch_size, dim)
        x = x + self.byte_pos_emb.to(dtype=x.dtype)[None, :, :]

        if self.mode == "mixer":
            x = x + self.dw_conv(x.transpose(1, 2)).transpose(1, 2)
            y = F.leaky_relu(self.mix_in(x), negative_slope=0.5)
            x = x + self.mix_out(y.square())
            x = x.reshape(bsz, num_patches, patch_size * dim)
            return self.mix_proj(x)

        qkv = self.qkv(x)
        q, k, v = qkv.chunk(3, dim=-1)
        q = q.reshape(bsz * num_patches, patch_size, self.num_heads, self.head_dim).transpose(1, 2)
        k = k.reshape(bsz * num_patches, patch_size, self.num_heads, self.head_dim).transpose(1, 2)
        v = v.reshape(bsz * num_patches, patch_size, self.num_heads, self.head_dim).transpose(1, 2)
        scores = torch.matmul(q, k.transpose(-2, -1)) * (self.head_dim ** -0.5)
        scores = scores.masked_fill(self.attn_mask[None, None, :, :], float("-inf"))
        attn = torch.matmul(scores.softmax(dim=-1), v)
        attn = attn.transpose(1, 2).reshape(bsz * num_patches, patch_size, dim)
        x = x + self.attn_out(attn)
        y = F.leaky_relu(self.attn_ff_in(x), negative_slope=0.5)
        x = x + self.attn_ff_out(y.square())
        return x[:, -1, :].reshape(bsz, num_patches, dim)


class PatchFirstJEPA(nn.Module):
    def __init__(
        self,
        *,
        vocab_size: int,
        ...
        self.patch_encoder = PatchReducer(model_dim, patch_size, patch_embed_mode, num_heads)
        self.patch_bos = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))
        self.local_bos = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))
        self.local_pos_emb = nn.Parameter(torch.zeros(patch_size, model_dim, dtype=torch.float32))
        self.decoder_in = naivebaseline.CastedLinear(model_dim * 3, model_dim, bias=False)
        self.decoder_mid = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
        self.decoder_out = naivebaseline.CastedLinear(model_dim, model_dim, bias=False)
        self.decoder_out._zero_init = True
    ...
    def _init_weights(self) -> None:
        ...
        nn.init.normal_(self.local_bos, mean=0.0, std=self.tied_embed_init_std)
        nn.init.normal_(self.local_pos_emb, mean=0.0, std=self.tied_embed_init_std)
        for module in self.modules():
            if isinstance(module, nn.Linear) and getattr(module, "_zero_init", False):
                nn.init.zeros_(module.weight)

    def _patchify_tokens(self, input_ids: Tensor) -> Tensor:
    ...
    def encode_patch_context(self, input_ids: Tensor) -> Tensor:
        x = self._patchify_tokens(input_ids)
    ...
    def decode_byte_logits(self, patch_hidden: Tensor, target_ids: Tensor) -> Tensor:
        bsz, num_patches, dim = patch_hidden.shape
        target_patches = target_ids.reshape(bsz, num_patches, self.patch_size)
        patch_context = F.rms_norm(patch_hidden, (dim,)).reshape(bsz * num_patches, dim)
        prev = self.local_bos.to(dtype=patch_hidden.dtype)[None, :].expand(bsz * num_patches, -1)
        logits_steps: list[Tensor] = []
        flat_target = target_patches.reshape(bsz * num_patches, self.patch_size)
        for step in range(self.patch_size):
            pos = self.local_pos_emb[step].to(dtype=patch_hidden.dtype)[None, :].expand_as(prev)
            hidden = torch.cat((patch_context, prev, pos), dim=-1)
            hidden = F.leaky_relu(self.decoder_in(hidden), negative_slope=0.5)
            hidden = self.decoder_mid(hidden.square())
            hidden = patch_context + self.decoder_out(hidden)
            logits_steps.append(self._logits_from_hidden(hidden))
            if step + 1 < self.patch_size:
                prev_tokens = flat_target[:, step]
                prev = self.tok_emb(prev_tokens)
        return torch.stack(logits_steps, dim=1).reshape(bsz * num_patches * self.patch_size, -1)
...
@torch.no_grad()
def sync_target_modules(
    target_tok_emb: nn.Embedding,
    online_tok_emb: nn.Embedding,
    target_patch_encoder: nn.Module,
    online_patch_encoder: nn.Module,
) -> None:
    target_tok_emb.weight.copy_(online_tok_emb.weight.detach().float())
    target_patch_encoder.load_state_dict(
        {k: v.detach().float().clone() for k, v in online_patch_encoder.state_dict().items()},
        strict=True,
    )


@torch.no_grad()
def update_target_modules_ema(
    target_tok_emb: nn.Embedding,
    online_tok_emb: nn.Embedding,
    target_patch_encoder: nn.Module,
    online_patch_encoder: nn.Module,
    ...
    target_state = target_patch_encoder.state_dict()
    online_state = online_patch_encoder.state_dict()
    for name, tensor in target_state.items():
        tensor.mul_(decay).add_(online_state[name].detach().float(), alpha=1.0 - decay)


def clear_rotary_caches(module: nn.Module) -> None:
    for submodule in module.modules():
        rotary = getattr(submodule, "rotary", None)
        if rotary is None:
            continue
        if hasattr(rotary, "_cos_cached"):
            rotary._cos_cached = None
        if hasattr(rotary, "_sin_cached"):
            rotary._sin_cached = None
        if hasattr(rotary, "_seq_len_cached"):
...
def main() -> None:
    ...
    base_model = PatchFirstJEPA(
        ...
        patch_embed_mode=args.patch_embed_mode,
    ...
    target_patch_encoder = PatchReducer(
        args.model_dim,
        args.patch_size,
        args.patch_embed_mode,
        args.num_heads,
    ).to(device=device, dtype=torch.float32)
    sync_target_modules(target_tok_emb, base_model.tok_emb, target_patch_encoder, base_model.patch_encoder)
    ...
    patch_matrix_params = [p for p in base_model.patch_encoder.parameters() if p.ndim == 2]
    patch_scalar_params = [p for p in base_model.patch_encoder.parameters() if p.ndim < 2]
    matrix_params.extend(
        patch_matrix_params
        + [
            base_model.decoder_in.weight,
            base_model.decoder_mid.weight,
            base_model.decoder_out.weight,
            base_model.jepa_in.weight,
            base_model.jepa_out.weight,
        ]
    )
    scalar_params = [
        p
    ...
    scalar_params.extend(
        patch_scalar_params + [
            base_model.patch_bos,
            base_model.local_bos,
            base_model.local_pos_emb,
        ]
    )
    ...
    log0(
        f"model_family:patch_first_jepa patch_size:{args.patch_size} "
    ...
    log0(f"patch_embed_mode:{args.patch_embed_mode}")
    log0("decoder_mode:patch_shifted_conditional_mlp")
    ...
    while True:
        ...
        for opt in optimizers:
            opt.step()
        update_target_modules_ema(target_tok_emb, base_model.tok_emb, target_patch_encoder, base_model.patch_encoder, args.target_ema)
