...
PROGRAM_PATH="$ROOT_DIR/records/byte-level-jepa/target-embedding-autoresearch/program.md"
...
if [ "$BACKGROUND" = "1" ]; then
  ...
  env "${COMMON_ENV[@]}" nohup "$PYTHON_BIN" -u autoresearch.py >"$LOG_FILE" 2>&1 &
  echo "Started in background: $!"
  echo "Log: $LOG_FILE"
else
  exec env "${COMMON_ENV[@]}" "$PYTHON_BIN" -u autoresearch.py
fi
