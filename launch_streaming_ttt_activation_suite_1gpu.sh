...
LEADER_SCRIPT_PATH="$REPO_ROOT/records/track_10min_16mb/2026-03-23_LeakyReLU_LegalTTT_ParallelMuon/train_gpt.py"
CODEX_SCRIPT_PATH="$REPO_ROOT/records/track_10min_16mb/2026-03-24_StreamingTTT_Candidate/train_gpt.py"
...
SUITE_ROOT="${SUITE_ROOT:-$RECORD_ROOT/bench_1gpu_activation_suite}"
USER_TTT_LR="${TTT_LR-}"
USER_TTT_EPOCHS="${TTT_EPOCHS-}"
USER_TTT_CHUNK_TOKENS="${TTT_CHUNK_TOKENS-}"
USER_TTT_LR_SCHEDULE="${TTT_LR_SCHEDULE-}"
USER_TTT_LAST_N_BLOCKS="${TTT_LAST_N_BLOCKS-}"
USER_ACTIVATION_NEG_SLOPE="${ACTIVATION_NEG_SLOPE-}"
USER_ASYMMETRIC_SQUARE_INIT="${ASYMMETRIC_SQUARE_INIT-}"
USER_GATED_SQUARE_BETA_INIT="${GATED_SQUARE_BETA_INIT-}"
USER_VAL_TOKENS_LIMIT="${VAL_TOKENS_LIMIT-}"
...
export VAL_TOKENS_LIMIT="${USER_VAL_TOKENS_LIMIT:-1048576}"
...
CASE_NAMES=(
  leader_leaky_relu_sq
  leader_asymmetric_square
  leader_gated_square
  leader_sign_preserving_square
  codex_leaky_relu_sq
  codex_asymmetric_square
  codex_gated_square
  codex_sign_preserving_square
)
...
configure_case() {
  ...
  unset RUN_ID SEED
  unset TTT_MODE TTT_PARAM_MODE TTT_FREEZE_BLOCKS TTT_LR TTT_EPOCHS TTT_CHUNK_TOKENS TTT_LR_SCHEDULE TTT_LAST_N_BLOCKS
  unset ACTIVATION_MODE ACTIVATION_NEG_SLOPE ASYMMETRIC_SQUARE_INIT GATED_SQUARE_BETA_INIT

  export ACTIVATION_NEG_SLOPE="${USER_ACTIVATION_NEG_SLOPE:-0.5}"
  export ASYMMETRIC_SQUARE_INIT="${USER_ASYMMETRIC_SQUARE_INIT:-0.25}"
  export GATED_SQUARE_BETA_INIT="${USER_GATED_SQUARE_BETA_INIT:-1.0}"

  case "$case_name" in
    leader_*)
      SCRIPT_PATH="$LEADER_SCRIPT_PATH"
      export TTT_MODE=chunk
      export TTT_PARAM_MODE=freeze_first_n
      export TTT_FREEZE_BLOCKS=0
      export TTT_LR="${USER_TTT_LR:-0.002}"
      export TTT_EPOCHS="${USER_TTT_EPOCHS:-3}"
      export TTT_CHUNK_TOKENS="${USER_TTT_CHUNK_TOKENS:-32768}"
      export TTT_LR_SCHEDULE="${USER_TTT_LR_SCHEDULE:-chunk_cosine}"
      ;;
    codex_*)
      SCRIPT_PATH="$CODEX_SCRIPT_PATH"
      export TTT_MODE=stream
      export TTT_PARAM_MODE=late_blocks
      export TTT_LAST_N_BLOCKS="${USER_TTT_LAST_N_BLOCKS:-4}"
      export TTT_LR="${USER_TTT_LR:-0.0005}"
      export TTT_EPOCHS="${USER_TTT_EPOCHS:-1}"
      export TTT_CHUNK_TOKENS="${USER_TTT_CHUNK_TOKENS:-32768}"
      export TTT_LR_SCHEDULE="${USER_TTT_LR_SCHEDULE:-constant}"
      ;;
    *)
      echo "Unknown case group in $case_name"
      exit 1
      ;;
  esac

  case "$case_name" in
    *_leaky_relu_sq) export ACTIVATION_MODE=leaky_relu_sq ;;
    *_asymmetric_square) export ACTIVATION_MODE=asymmetric_square ;;
    *_gated_square) export ACTIVATION_MODE=gated_square ;;
    *_sign_preserving_square) export ACTIVATION_MODE=sign_preserving_square ;;
    *)
      echo "Unknown activation in $case_name"
      exit 1
      ;;
  esac
}
