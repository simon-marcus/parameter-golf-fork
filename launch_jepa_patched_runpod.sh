...
SCRIPT_PATH="/workspace/parameter-golf/train_jepa_patched.py"
RECORD_ROOT="/workspace/parameter-golf/records/nonrecord_jepa_patched"
...
case "$VARIANT" in
  control)
    export RUN_ID="${RUN_ID:-jepa_patched${MODE_SUFFIX}_control_runpod}"
    export JEPA_LOSS_WEIGHT=0.0
    OUT_DIR="$RECORD_ROOT/${DATA_ROOT_MODE}${MODE_SUFFIX}_control"
    ;;
  jepa01)
    export RUN_ID="${RUN_ID:-jepa_patched${MODE_SUFFIX}_jepa01_runpod}"
    export JEPA_LOSS_WEIGHT=0.1
    OUT_DIR="$RECORD_ROOT/${DATA_ROOT_MODE}${MODE_SUFFIX}_jepa01"
    ;;
  jepa02)
    export RUN_ID="${RUN_ID:-jepa_patched${MODE_SUFFIX}_jepa02_runpod}"
    export JEPA_LOSS_WEIGHT=0.2
    OUT_DIR="$RECORD_ROOT/${DATA_ROOT_MODE}${MODE_SUFFIX}_jepa02"
    ;;
  weight:*)
    WEIGHT_VALUE="${VARIANT#weight:}"
    WEIGHT_TAG="${WEIGHT_VALUE//./p}"
    export RUN_ID="${RUN_ID:-jepa_patched${MODE_SUFFIX}_w${WEIGHT_TAG}_runpod}"
...

{
  ...
  echo "PATCH_EMBED_MODE=$PATCH_EMBED_MODE"
