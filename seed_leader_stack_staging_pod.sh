...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 <prep_host> <prep_port> <target_host> <target_port>" >&2
  exit 1
fi
...
PREP_REPO_ROOT="/workspace/pg-data/parameter-golf"
BUNDLE_PATH="$PREP_REPO_ROOT/staging_bundles/parameter-golf-leader-stack-jepa-bundle.tar"
ARCHIVE_PATH="$PREP_REPO_ROOT/data/archives/fineweb10B_sp1024__fineweb_1024_bpe.tar.zst"
...
ssh -o StrictHostKeyChecking=no "root@${TARGET_HOST}" -p "$TARGET_PORT" \
  "mkdir -p '$REPO_ROOT' '$TARGET_ARCHIVE_DIR' '$TARGET_RECORD_DIR'"

ssh -o StrictHostKeyChecking=no "root@${PREP_HOST}" -p "$PREP_PORT" "cat '$BUNDLE_PATH'" \
  | ssh -o StrictHostKeyChecking=no "root@${TARGET_HOST}" -p "$TARGET_PORT" \
...
"$(cd "$(dirname "$0")" && pwd)/relay_runpod_archive.sh" \
  "$PREP_HOST" "$PREP_PORT" "$ARCHIVE_PATH" \
