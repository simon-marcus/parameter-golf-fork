...
RECORD_ROOT="/workspace/parameter-golf/records/track_10min_16mb/2026-03-31_baseline_legal_ttt_runpod"
WORKSPACE_BUNDLE_ROOT="${WORKSPACE_BUNDLE_ROOT:-/workspace/parameter-golf/baseline_smoke_bundle}"
TMP_BUNDLE_ROOT="/tmp/parameter-golf-baseline"
DATASET_NAME="${DATASET_NAME:-fineweb10B_sp1024}"
TOKENIZER_STEM="${TOKENIZER_STEM:-fineweb_1024_bpe}"
EXPECTED_TRAIN_SHARDS="${EXPECTED_TRAIN_SHARDS:-2}"
...
case "$DATA_ROOT_MODE" in
  workspace)
    ...
    export TOKENIZER_PATH="$WORKSPACE_BUNDLE_ROOT/tokenizers/$TOKENIZER_STEM.model"
    export TOKENIZER_META_PATH="$WORKSPACE_BUNDLE_ROOT/tokenizers/$TOKENIZER_STEM.meta.npz"
    ;;
  tmp)
    ...
    export TOKENIZER_PATH="$TMP_BUNDLE_ROOT/tokenizers/$TOKENIZER_STEM.model"
    export TOKENIZER_META_PATH="$TMP_BUNDLE_ROOT/tokenizers/$TOKENIZER_STEM.meta.npz"
    ;;
  *)
...
export RUN_ID="${RUN_ID:-baseline_legal_ttt_runpod}"
