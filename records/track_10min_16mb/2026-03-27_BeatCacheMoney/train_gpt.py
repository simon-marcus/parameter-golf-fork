...
try:
    ...
    def flash_attn_3_func(q, k, v, causal=True):
        # Fallback to PyTorch SDPA: flash_attn uses (B,T,H,D), SDPA uses (B,H,T,D)
        q2 = q.transpose(1, 2)
        k2 = k.transpose(1, 2)
        v2 = v.transpose(1, 2)
        o = F.scaled_dot_product_attention(q2, k2, v2, is_causal=causal, enable_gqa=(k2.size(1) != q2.size(1)))
...
class Hyperparameters:
    ...
    ngram_max_order = int(os.environ.get("NGRAM_MAX_ORDER", 20))
    ...
    ngram_alpha_max = float(os.environ.get("NGRAM_ALPHA_MAX", 0.80))
    ...
    phrase_probe_lengths = os.environ.get("PHRASE_PROBE_LENGTHS", "64,56,48,36,28,20,16")
    phrase_num_buckets = int(os.environ.get("PHRASE_NUM_BUCKETS", 4_194_304))  # 4M per length
    ...
    ngram_calibration_alpha_max_grid = os.environ.get("NGRAM_CALIBRATION_ALPHA_MAX_GRID", "0.70,0.80,0.90,0.95,0.99")
    ngram_calibration_entropy_center_grid = os.environ.get("NGRAM_CALIBRATION_ENTROPY_CENTER_GRID", "2.0,2.5,3.0,3.5")
    phrase_calibration_alpha_max_grid = os.environ.get("PHRASE_CALIBRATION_ALPHA_MAX_GRID", "0.980,0.990,0.995,0.999")
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True) if bool(int(os.environ.get("USE_COMPILE", "1"))) else base_model.forward_logits
    with torch.inference_mode():
...
# Per-order multipliers: orders 2-3 suppressed, 4 near-neutral, 5+ boosted
...
class PhraseCache:
    """Separate per-length phrase tables for long-range repetition detection."""
    ...
    def __init__(self, probe_lengths: list[int], num_buckets: int = 4_194_304):
        ...
        self.bucket_mask = np.int64(num_buckets - 1)
        self.ctx_tables = {
            L: np.zeros(num_buckets, dtype=np.int32) for L in self.probe_lengths
        }
        self.full_tables = {
            L: np.zeros(num_buckets, dtype=np.int32) for L in self.probe_lengths
        }
...
def _blend_cache_probs(
    ...
    """Two-stage probability blending: n-gram then phrase on top."""
...
def _calibrate_cache_blend(
    ...
    """Grid search calibration for blend hyperparameters."""
    if len(p_model) == 0:
        return args.ngram_alpha_max, args.ngram_entropy_center, args.phrase_alpha_max

    alpha_grid = _parse_float_grid(
        args.ngram_calibration_alpha_max_grid, (args.ngram_alpha_max,))
    center_grid = _parse_float_grid(
        args.ngram_calibration_entropy_center_grid, (args.ngram_entropy_center,))
    phrase_alpha_grid = _parse_float_grid(
        args.phrase_calibration_alpha_max_grid, (args.phrase_alpha_max,)
    ...
    for alpha_max in alpha_grid:
        for center in center_grid:
            for pa_max in phrase_alpha_grid:
                p_blend = _blend_cache_probs(
                    args, p_model, entropy, ngram_prob, matched_order,
                    phrase_prob, matched_len, phrase_probe_lengths,
                    alpha_max, center, pa_max,
                )
                mean_nll = float((-np.log(p_blend)).mean())
                if mean_nll < best_nll:
                    best_nll = mean_nll
                    best = (alpha_max, center, pa_max)

    log0(
        f"calibration: tokens={len(p_model)} grid={len(alpha_grid)}x{len(center_grid)}x{len(phrase_alpha_grid)} "
...
def ngram_rescore(
    ...
    """Rescore tokens using n-gram + phrase cache blended with stored neural model_p.

    This is Pass 2: caches are already complete, calibration is done inline.
    ...
    # --- Score n-gram cache (with LOO) ---
    ngram_prob, matched_order = ngram_cache.score_positions(
        tokens_np, positions,
        min_count=args.ngram_min_count,
        leave_one_out=args.ngram_leave_one_out,
    )
    n_ng_matched = int((matched_order >= 0).sum())

    # --- Score phrase cache (with LOO) ---
    phrase_prob = None
    matched_len = None
    phrase_probe_lengths: list[int] = []
    n_ph_matched = 0
    if phrase_cache is not None and args.phrase_enabled:
        phrase_probe_lengths = phrase_cache.probe_lengths
        phrase_prob, matched_len = phrase_cache.score_positions(
            tokens_np, positions,
            min_count=args.phrase_min_count,
            leave_one_out=args.ngram_leave_one_out,
        )
        n_ph_matched = int((matched_len > 0).sum())

    # --- Calibrate blend parameters on first fraction of tokens ---
    cal_alpha_max = args.ngram_alpha_max
    cal_center = args.ngram_entropy_center
    cal_phrase_max = args.phrase_alpha_max
    if args.ngram_calibration_enabled and N > 100:
        cal_n = max(100, int(N * args.ngram_calibration_frac))
        cal_alpha_max, cal_center, cal_phrase_max = _calibrate_cache_blend(
            args,
            ...
            phrase_probe_lengths, log0=log0,
        )

    # --- Blend all tokens with calibrated parameters ---
    p_blend = _blend_cache_probs(
        args, model_p, entropy, ngram_prob, matched_order,
        phrase_prob, matched_len, phrase_probe_lengths,
        cal_alpha_max, cal_center, cal_phrase_max,
    )
    ...
    log0(
        f"ngram_rescore: ngram_matched={n_ng_matched}/{N} ({100*n_ng_matched/max(N,1):.1f}%) "
        f"phrase_matched={n_ph_matched}/{N} ({100*n_ph_matched/max(N,1):.1f}%) "
        f"cal_alpha_max={cal_alpha_max:.3f} cal_center={cal_center:.1f} "
        f"cal_phrase_max={cal_phrase_max:.3f} loo={int(args.ngram_leave_one_out)}"
    )
...
def eval_ngram_two_pass(
    ...
    """Two-pass n-gram + phrase evaluation with calibration.

    Pass 1: Sliding-window neural eval (T=0.85) → store per-token model_p and entropy.
    Build:  Complete n-gram cache + phrase cache from all val tokens.
    Calibrate: Grid-search blend hyperparameters on first 5% of tokens.
    Pass 2: Rescore ALL tokens with calibrated n-gram + phrase blending.
    """
    t0 = time.perf_counter()

    # --- Pass 1: Neural eval with per-token storage (or load from cache) ---
    pass1_load = os.environ.get("PASS1_LOAD_PATH", "")
    pass1_save = os.environ.get("PASS1_SAVE_PATH", "")
    if pass1_load and os.path.exists(pass1_load):
        log0(f"ngram_two_pass: loading Pass 1 arrays from {pass1_load}")
        d = np.load(pass1_load)
        model_p, entropy, token_bytes, positions = d["model_p"], d["entropy"], d["token_bytes"], d["positions"]
        pass1_bpb = float(d["pass1_bpb"])
        log0(f"ngram_two_pass: loaded {len(positions)} scored tokens, pass1_bpb={pass1_bpb:.6f}")
    else:
        log0(f"ngram_two_pass: starting Pass 1 (sliding-window neural eval, T={args.ngram_temperature})")
        ...
        if pass1_save and rank == 0:
            tokens_for_save = val_tokens.numpy().astype(np.int16)
            np.savez(pass1_save, model_p=model_p, entropy=entropy,
                     token_bytes=token_bytes, positions=positions,
                     pass1_bpb=np.array(pass1_bpb),
                     val_tokens=tokens_for_save)
            log0(f"ngram_two_pass: saved Pass 1 arrays to {pass1_save} "
                 f"({len(positions)} positions, use NPROC=1 for complete data)")
    ...
    ngram_cache.build_full(tokens_np)
    t_ng = time.perf_counter()
    log0(f"ngram_two_pass: n-gram cache built in {t_ng - t_pass1:.1f}s")

    # --- Build phrase cache ---
    phrase_cache = None
    if args.phrase_enabled:
        probe_lengths = [int(x.strip()) for x in args.phrase_probe_lengths.split(",") if x.strip()]
        log0(f"ngram_two_pass: building phrase cache lengths={probe_lengths} "
             f"buckets={args.phrase_num_buckets}")
    ...
    log0(f"ngram_two_pass: phrase cache built in {t_ph - t_ng:.1f}s")

    # --- Pass 2: Rescore with n-gram + phrase + calibration ---
    log0(f"ngram_two_pass: starting Pass 2 (rescore with calibration)")
    val_loss, val_bpb = ngram_rescore(
        args, tokens_np, ngram_cache, phrase_cache,
        model_p, entropy, token_bytes, positions,
        rank, world_size, device, log0=log0,
    )
    t_pass2 = time.perf_counter()
    log0(f"ngram_two_pass: Pass 2 done val_bpb={val_bpb:.6f} "
         f"improvement={pass1_bpb - val_bpb:.6f} time={t_pass2 - t_ph:.1f}s")
...
def main() -> None:
    ...
    eval_only_model_path = os.environ.get("EVAL_ONLY_MODEL", "final_model.int6.ptz")
    if eval_only:
        log0(f"EVAL_ONLY: skipping training, loading {eval_only_model_path}")
        # Jump straight to quantized model loading and eval
        CastedLinear._qat_enabled = False
        # Load quantized model
        with open(eval_only_model_path, "rb") as f:
            quant_blob_disk = f.read()
        quant_state = torch.load(
            io.BytesIO(lzma.decompress(quant_blob_disk)),
            map_location="cpu",
        )
        # Need a dummy unbanked_sd for dequantization shape info
        dummy_model = GPT(
        ...
        dummy_sd = {k: v.detach().cpu() for k, v in dummy_model.state_dict().items() if "mtp_heads" not in k}
        unbanked_sd = _unbank_state_dict(dummy_sd, args.num_layers)
        deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], unbanked_sd)
        deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, dummy_sd)
        del dummy_model, dummy_sd
        eval_model = GPT(
            vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
            num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
            tie_embeddings=args.tie_embeddings, tied_embed_init_std=args.tied_embed_init_std,
        ...
        log0(f"EVAL_ONLY: model loaded, starting n-gram eval")
        # Jump to n-gram eval
    ...
    compiled_forward_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True) if _use_compile else base_model.forward_logits
    ...
    restore_low_dim_params_to_fp32(eval_model)
    eval_model.load_state_dict(deq_state, strict=True)
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=True) if bool(int(os.environ.get("USE_COMPILE", "1"))) else eval_model
    torch.cuda.synchronize()
