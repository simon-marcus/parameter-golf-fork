...
class Hyperparameters:
    ...
    ttt_block_start = int(os.environ.get("TTT_BLOCK_START", 0))
    ttt_block_end = int(os.environ.get("TTT_BLOCK_END", -1))
...
def _setup_ttt_params(
    ...
    block_start = min(max(args.ttt_block_start, 0), num_layers)
    block_end = num_layers if args.ttt_block_end < 0 else min(max(args.ttt_block_end, 0), num_layers)
    keep_block_range_ids = set(range(block_start, max(block_start, block_end)))
    ...
    if param_mode == "full":
        ...
        mode_desc = "full"
    elif param_mode == "block_range":
        keep_block_ids = keep_block_range_ids
        include_non_block = False
        mode_desc = f"block_range:{block_start}-{max(block_start, block_end)}"
    else:
    ...
    for name, p in base_model.named_parameters():
        ...
        if bank_mask is not None:
            if bank_mask.sum().item() > 0:
                keep_param = True
                grad_masks[id(p)] = bank_mask.to(dtype=p.dtype)
        elif name.startswith("blocks."):
            block_idx = int(name.split(".")[1])
            if block_idx in keep_block_ids:
                keep_param = True
...
def eval_val_sliding_ttt(
    ...
    """Legal score-first chunked TTT over the eval token stream."""
    ...
    log0(
        f"ttt:start mode=chunk param_mode={mode_desc} chunks={num_chunks} "
    ...
    base_model.eval()
    log0(
        f"ttt:done mode=chunk val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
...
def main() -> None:
    ...
    log0(f"activation_mode:leaky_relu_sq neg_slope:{args.activation_neg_slope}")
    log0(
