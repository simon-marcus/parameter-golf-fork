...
class Hyperparameters:
    ...
    ngram_mode = os.environ.get("NGRAM_MODE", "two_pass")
    ngram_online_chunk_tokens = int(os.environ.get("NGRAM_ONLINE_CHUNK_TOKENS", 131072))
    ...
    ngram_temperature = float(os.environ.get("NGRAM_TEMPERATURE", 0.85))
    ngram_calibration_enabled = bool(int(os.environ.get("NGRAM_CALIBRATION_ENABLED", "1")))
    ngram_calibration_frac = float(os.environ.get("NGRAM_CALIBRATION_FRAC", 0.05))
    ngram_calibration_alpha_max_grid = os.environ.get("NGRAM_CALIBRATION_ALPHA_MAX_GRID", "0.90,0.94,0.97,0.99")
    ngram_calibration_entropy_center_grid = os.environ.get("NGRAM_CALIBRATION_ENTROPY_CENTER_GRID", "2.5,3.0,3.5")
    phrase_enabled = bool(int(os.environ.get("PHRASE_ENABLED", "1")))
    phrase_probe_lengths = os.environ.get("PHRASE_PROBE_LENGTHS", "64,48,32,16")
    phrase_num_buckets = int(os.environ.get("PHRASE_NUM_BUCKETS", 8_388_608))
    phrase_min_count = int(os.environ.get("PHRASE_MIN_COUNT", 1))
    phrase_alpha_min = float(os.environ.get("PHRASE_ALPHA_MIN", 0.88))
    phrase_alpha_max = float(os.environ.get("PHRASE_ALPHA_MAX", 0.995))
    phrase_entropy_center = float(os.environ.get("PHRASE_ENTROPY_CENTER", 2.5))
    phrase_entropy_scale = float(os.environ.get("PHRASE_ENTROPY_SCALE", 2.0))
    phrase_calibration_alpha_max_grid = os.environ.get("PHRASE_CALIBRATION_ALPHA_MAX_GRID", "0.980,0.990,0.995")
    artifact_ngram_export = bool(int(os.environ.get("ARTIFACT_NGRAM_EXPORT", "1")))
    artifact_ngram_min_order = int(os.environ.get("ARTIFACT_NGRAM_MIN_ORDER", 2))
    artifact_ngram_max_order = int(os.environ.get("ARTIFACT_NGRAM_MAX_ORDER", 9))
    artifact_ngram_buckets = int(os.environ.get("ARTIFACT_NGRAM_BUCKETS", 32768))
    artifact_gate_strength = float(os.environ.get("ARTIFACT_GATE_STRENGTH", 1.0))
...
class NgramCache:
    ...
    def seed_from_packed(self, packed: dict[str, object]):
        if int(packed.get("min_order", self.min_order)) != self.min_order:
            raise ValueError("packed min_order mismatch")
        if int(packed.get("max_order", self.max_order)) != self.max_order:
            raise ValueError("packed max_order mismatch")
        if int(packed.get("num_buckets", self.num_buckets)) != self.num_buckets:
            raise ValueError("packed bucket count mismatch")
        ctx_tables = packed["ctx_tables"]
        full_tables = packed["full_tables"]
        for oi in range(self.num_orders):
            self.ctx_tables[oi] = np.asarray(ctx_tables[oi], dtype=np.int32).copy()
            self.full_tables[oi] = np.asarray(full_tables[oi], dtype=np.int32).copy()
    ...
    def score_positions(self, tokens_np: np.ndarray, positions: np.ndarray,
        ...
        for oi in range(self.num_orders - 1, -1, -1):
            ...
            for k in range(n - 1):
                h ^= tokens_np[pos_valid - (n - 1) + k].astype(np.int64) * _NGRAM_PRIMES[k % len(_NGRAM_PRIMES)]
            ctx_h = h & self.bucket_mask
            target_prime = _NGRAM_PRIMES[min(n - 1, len(_NGRAM_PRIMES) - 1)]
...
class PhraseCache:
    """Separate per-length phrase tables to reduce cross-length contamination."""

    def __init__(self, probe_lengths: list[int], num_buckets: int = 8_388_608):
    ...
    def _compute_hashes(self, tokens_np: np.ndarray, start: int, end: int, length: int):
        valid_start = max(start, length)
        N = end - valid_start
        if N <= 0:
            return None, None, valid_start
        h = np.zeros(N, dtype=np.int64)
    ...
    def build_full(self, tokens_np: np.ndarray):
        for length in self.probe_lengths:
            ctx_h, full_h, _ = self._compute_hashes(tokens_np, 0, len(tokens_np), length)
            if ctx_h is None:
                continue
    ...
    def score_positions(self, tokens_np: np.ndarray, positions: np.ndarray,
                        min_count: int = 1, leave_one_out: bool = False):
        N = len(positions)
        ...
        for length in self.probe_lengths:
            ...
            out_idx = valid_idx[eligible]
            phrase_prob[out_idx] = (
                full_counts[eligible].astype(np.float32)
                / np.maximum(ctx_counts[eligible].astype(np.float32), 1.0)
            )
...
def eval_val_sliding_store(
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            logits_f = logits.float() / max(args.ngram_temperature, 1e-4)
...
def _blend_cache_probs(
    args: Hyperparameters,
    p_model: np.ndarray,
    entropy: np.ndarray,
    ngram_prob: np.ndarray,
    matched_order: np.ndarray,
    phrase_prob: np.ndarray | None,
    matched_len: np.ndarray | None,
    phrase_probe_lengths: list[int],
    ngram_alpha_max: float,
    ngram_entropy_center: float,
    phrase_alpha_max: float,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    p_blend = p_model.copy()
    ngram_alpha = np.zeros(len(p_model), dtype=np.float32)
    phrase_alpha = np.zeros(len(p_model), dtype=np.float32)

    matched = matched_order >= 0
    if np.any(matched):
        order_idx = (matched_order[matched] - args.ngram_min_order).astype(np.int32)
        ...
        raw_alpha = args.ngram_alpha_min + (ngram_alpha_max - args.ngram_alpha_min) * sig
        mults = _ORDER_MULTS[np.minimum(order_idx, len(_ORDER_MULTS) - 1)]
        alpha = np.clip(raw_alpha * mults, 0.0, 0.98)
        ngram_alpha[matched] = alpha
    ...
    if phrase_prob is not None and matched_len is not None:
        ...
        if np.any(phrase_matched) and phrase_probe_lengths:
            ...
            sig = 1.0 / (1.0 + np.exp(
                -args.phrase_entropy_scale * (entropy[phrase_matched] - args.phrase_entropy_center)
            ))
            alpha = args.phrase_alpha_min + (phrase_alpha_max - args.phrase_alpha_min) * len_frac
            alpha = np.clip(alpha * (0.5 + 0.5 * sig), 0.0, 0.999)
            phrase_alpha[phrase_matched] = alpha
            p_blend[phrase_matched] = (
                (1.0 - alpha) * p_blend[phrase_matched] + alpha * phrase_prob[phrase_matched]
            )

    return np.clip(p_blend, 1e-10, 1.0), ngram_alpha, phrase_alpha


def _calibrate_cache_blend(
    args: Hyperparameters,
    p_model: np.ndarray,
    entropy: np.ndarray,
    ngram_prob: np.ndarray,
    ...
    alpha_grid = _parse_float_grid(args.ngram_calibration_alpha_max_grid, (args.ngram_alpha_max,))
    center_grid = _parse_float_grid(args.ngram_calibration_entropy_center_grid, (args.ngram_entropy_center,))
    phrase_alpha_grid = (
        _parse_float_grid(args.phrase_calibration_alpha_max_grid, (args.phrase_alpha_max,))
        if phrase_prob is not None else [args.phrase_alpha_max]
    )

    best = (args.ngram_alpha_max, args.ngram_entropy_center, args.phrase_alpha_max)
    best_nll = float("inf")
    for alpha_max in alpha_grid:
        for center in center_grid:
            for phrase_alpha_max in phrase_alpha_grid:
                p_blend, _, _ = _blend_cache_probs(
                    args,
                    p_model,
                    entropy,
                    ngram_prob,
                ...
                mean_nll = float((-np.log(p_blend)).mean())
                if mean_nll < best_nll:
                    best_nll = mean_nll
                    best = (alpha_max, center, phrase_alpha_max)

    log0(
        f"ngram_two_pass:calibration tokens={len(p_model)} "
        f"best_alpha_max={best[0]:.3f} best_entropy_center={best[1]:.3f} "
        f"best_phrase_alpha_max={best[2]:.3f} mean_nll={best_nll:.6f}"
    )
    return best


class ArtifactNgramTracker:
    """Collect a compact hashed n-gram memory during training for export in the artifact."""

    def __init__(self, min_order: int, max_order: int, num_buckets: int, device: torch.device):
        self.min_order = min_order
        self.max_order = max_order
        self.num_orders = max_order - min_order + 1
        self.num_buckets = num_buckets
        self.bucket_mask = int(num_buckets - 1)
        self.device = device
        self.ctx_tables = [torch.zeros(num_buckets, dtype=torch.int32, device=device) for _ in range(self.num_orders)]
        self.full_tables = [torch.zeros(num_buckets, dtype=torch.int32, device=device) for _ in range(self.num_orders)]

    @torch.no_grad()
    def update(self, x: Tensor, y: Tensor):
        x64 = x.to(dtype=torch.int64)
        y64 = y.to(dtype=torch.int64)
        _, seq_len = x64.shape
        for oi in range(self.num_orders):
            n = self.min_order + oi
            offset = n - 2
            if offset >= seq_len:
                continue
            span = seq_len - offset
            h = torch.zeros((x64.shape[0], span), dtype=torch.int64, device=x.device)
            for k in range(n - 1):
                h ^= x64[:, k:k + span] * int(_NGRAM_PRIMES[k % len(_NGRAM_PRIMES)])
            ctx_h = (h & self.bucket_mask).reshape(-1)
            target_prime = int(_NGRAM_PRIMES[min(n - 1, len(_NGRAM_PRIMES) - 1)])
            full_h = ((h ^ (y64[:, offset:] * target_prime)) & self.bucket_mask).reshape(-1)
            self.ctx_tables[oi] += torch.bincount(ctx_h, minlength=self.num_buckets).to(torch.int32)
            self.full_tables[oi] += torch.bincount(full_h, minlength=self.num_buckets).to(torch.int32)

    def finalize(self) -> dict[str, object]:
        if dist.is_available() and dist.is_initialized():
            for table in self.ctx_tables:
                dist.all_reduce(table, op=dist.ReduceOp.SUM)
        ...
        return {
            ...
            "ctx_tables": [torch.clamp(t, 0, 65535).to(dtype=torch.uint16).cpu() for t in self.ctx_tables],
            "full_tables": [torch.clamp(t, 0, 65535).to(dtype=torch.uint16).cpu() for t in self.full_tables],
        }
...
def ngram_rescore(
    ...
    phrase_prob = None
    matched_len = None
    phrase_probe_lengths = phrase_cache.probe_lengths if phrase_cache is not None else []
    ...
    active_ngram_alpha_max = args.ngram_alpha_max
    active_ngram_entropy_center = args.ngram_entropy_center
    active_phrase_alpha_max = args.phrase_alpha_max
    if args.ngram_calibration_enabled and N > 0:
        calib_n = max(1, min(N, int(N * args.ngram_calibration_frac)))
        active_ngram_alpha_max, active_ngram_entropy_center, active_phrase_alpha_max = _calibrate_cache_blend(
            args,
            model_p[:calib_n],
            entropy[:calib_n],
            ngram_prob[:calib_n],
            matched_order[:calib_n],
            phrase_prob[:calib_n] if phrase_prob is not None else None,
            matched_len[:calib_n] if matched_len is not None else None,
            phrase_probe_lengths,
            log0=log0,
        )

    p_blend, ngram_alpha, phrase_alpha = _blend_cache_probs(
        args,
        model_p,
        entropy,
        ngram_prob,
        ...
        matched_len,
        phrase_probe_lengths,
        active_ngram_alpha_max,
        active_ngram_entropy_center,
        active_phrase_alpha_max,
    )
    matched = matched_order >= 0
    ...
    phrase_matched = int((matched_len > 0).sum()) if matched_len is not None else 0
    mean_alpha = float(ngram_alpha[matched].mean()) if n_matched > 0 else 0.0
    mean_phrase_alpha = float(phrase_alpha[matched_len > 0].mean()) if phrase_matched > 0 else 0.0
    log0(
        f"ngram_rescore: matched={n_matched}/{N} ({100*n_matched/max(N,1):.1f}%) "
        f"mean_alpha={mean_alpha:.3f} phrase_matched={phrase_matched}/{N} "
        f"phrase_alpha={mean_phrase_alpha:.3f} "
        f"leave_one_out={int(args.ngram_leave_one_out)} "
        f"alpha_max={active_ngram_alpha_max:.3f} entropy_center={active_ngram_entropy_center:.3f} "
        f"phrase_alpha_max={active_phrase_alpha_max:.3f}"
        if n_matched > 0 or phrase_matched > 0
        else f"ngram_rescore: no matches leave_one_out={int(args.ngram_leave_one_out)}"
    )
...
def eval_ngram_two_pass(
    ...
    phrase_cache = None
    probe_lengths = [int(part.strip()) for part in args.phrase_probe_lengths.split(",") if part.strip()]
    if args.phrase_enabled and probe_lengths:
        phrase_cache = PhraseCache(
            probe_lengths=probe_lengths,
            num_buckets=args.phrase_num_buckets,
        )
        phrase_cache.build_full(tokens_np)
    t_cache = time.perf_counter()
    log0(
        f"ngram_two_pass: cache built in {t_cache - t_pass1:.1f}s "
        f"phrase_lengths={probe_lengths if phrase_cache is not None else []}"
    )

    # --- Pass 2: N-gram rescore ---
    log0(f"ngram_two_pass: starting Pass 2 (n-gram rescore)")
    val_loss, val_bpb = ngram_rescore(
        args, tokens_np, cache, phrase_cache, model_p, entropy, token_bytes, positions,
...
def eval_ngram_packed_online(
    args: Hyperparameters, base_model: nn.Module, packed_aux: dict[str, object], rank: int, world_size: int,
    device: torch.device, val_tokens: Tensor, base_bytes_lut: Tensor,
    has_leading_space_lut: Tensor, is_boundary_token_lut: Tensor,
    stride: int, batch_seqs: int = 32, log0=print,
) -> tuple[float, float]:
    """Single-pass causal eval seeded from an artifact-packed training cache."""
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    tokens_np = val_tokens.cpu().numpy().astype(np.int16, copy=False)
    online_chunk = max(args.ngram_online_chunk_tokens, stride)
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= stride or ws == 0]
    num_chunks = (total_tokens + online_chunk - 1) // online_chunk
    chunk_windows: list[list[int]] = [[] for _ in range(num_chunks)]
    for ws in window_starts:
        end_pos = min(ws + seq_len, total_tokens)
        wlen = end_pos - ws
        s = 0 if ws == 0 else max(wlen - stride, 0)
        scored_start = ws + s
        ci = min(scored_start // online_chunk, num_chunks - 1)
        chunk_windows[ci].append(ws)

    cache = NgramCache(
        min_order=int(packed_aux["min_order"]),
        max_order=int(packed_aux["max_order"]),
        num_buckets=int(packed_aux["num_buckets"]),
    )
    cache.seed_from_packed(packed_aux)
    ...
    t0 = time.perf_counter()
    log0(
        f"ngram_packed_online:start chunks={num_chunks} chunk_tokens={online_chunk} "
        f"orders={cache.min_order}-{cache.max_order} buckets={cache.num_buckets}"
    )
    ...
    with torch.inference_mode():
        for ci in range(num_chunks):
            ...
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                mp = probs.gather(2, y_batch.unsqueeze(-1)).squeeze(-1)
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    if seg_len <= 0:
                        continue
                    positions = np.arange(ws + s + 1, ws + wlen + 1, dtype=np.int64)
                    p_model = mp[i, s:wlen].cpu().numpy().astype(np.float32)
                    entropy = ent[i, s:wlen].cpu().numpy().astype(np.float32)
                    ngram_prob, matched_order = cache.score_positions(
                        tokens_np, positions, min_count=args.ngram_min_count, leave_one_out=False
                    )
                    p_blend, _, _ = _blend_cache_probs(
                        args,
                        p_model,
                        ...
                        None,
                        [],
                        min(args.ngram_alpha_max * args.artifact_gate_strength, 0.99),
                        args.ngram_entropy_center,
                        args.phrase_alpha_max,
                    )
                    loss_sum += torch.from_numpy(-np.log(p_blend).astype(np.float64)).to(device=device, dtype=torch.float64).sum()
            ...
            chunk_start = ci * online_chunk
            chunk_end = min((ci + 1) * online_chunk, total_tokens)
            cache.update_range(tokens_np, chunk_start, chunk_end + 1)
            if rank == 0 and (ci % 10 == 0 or ci == num_chunks - 1):
                elapsed = time.perf_counter() - t0
                rl = loss_sum.item() / max(token_count.item(), 1.0)
                rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1.0))
                log0(f"  ngram_packed_online [{ci+1}/{num_chunks}] bpb={rbpb:.6f} time={elapsed:.1f}s")
    ...
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    log0(f"ngram_packed_online:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} elapsed={time.perf_counter() - t0:.1f}s")
    return val_loss, val_bpb
...
def main() -> None:
    ...
    artifact_tracker = ArtifactNgramTracker(
        min_order=args.artifact_ngram_min_order,
        max_order=args.artifact_ngram_max_order,
        num_buckets=args.artifact_ngram_buckets,
        device=device,
    ) if args.artifact_ngram_export else None
    if bigram_tracker is not None:
        log0(f"complement:enabled alpha={args.complement_alpha}")
    if artifact_tracker is not None:
        log0(
            f"artifact_ngram:enabled orders={args.artifact_ngram_min_order}-{args.artifact_ngram_max_order} "
            f"buckets={args.artifact_ngram_buckets}"
        )
    ...
    packed_aux = artifact_tracker.finalize() if artifact_tracker is not None else None
    if master_process and packed_aux is not None:
        packed_bytes = sum(int(t.numel()) * int(t.element_size()) for t in packed_aux["ctx_tables"])
        packed_bytes += sum(int(t.numel()) * int(t.element_size()) for t in packed_aux["full_tables"])
        log0(f"artifact_ngram:packed_bytes_raw={packed_bytes}")
    if master_process:
    ...
    save_obj: dict[str, object] = {"w": quant_result, "m": quant_meta}
    if packed_aux is not None:
        save_obj["packed_aux"] = packed_aux
    ...
    packed_aux_loaded = quant_state.get("packed_aux")
    ...
    if args.ngram_enabled:
        ...
        if args.ngram_mode == "packed_online":
            if packed_aux_loaded is None:
                raise RuntimeError("NGRAM_MODE=packed_online requires packed_aux in artifact")
            ng_val_loss, ng_val_bpb = eval_ngram_packed_online(
                args, ngram_model, packed_aux_loaded, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                stride=args.eval_stride, log0=log0,
            )
            mode_name = "ngram_packed_online"
        else:
            ...
            mode_name = "ngram_two_pass"
        torch.cuda.synchronize()
        log0(f"{mode_name} val_loss:{ng_val_loss:.4f} val_bpb:{ng_val_bpb:.4f} "
             f"eval_time:{1000.0 * (time.perf_counter() - t_ngram):.0f}ms")
        log0(f"{mode_name}_exact val_loss:{ng_val_loss:.8f} val_bpb:{ng_val_bpb:.8f}")
