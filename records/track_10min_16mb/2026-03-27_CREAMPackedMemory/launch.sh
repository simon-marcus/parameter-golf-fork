...
export NGRAM_MODE="${NGRAM_MODE:-packed_online}"
export NGRAM_MIN_ORDER="${NGRAM_MIN_ORDER:-2}"
export NGRAM_MAX_ORDER="${NGRAM_MAX_ORDER:-9}"
export NGRAM_NUM_BUCKETS="${NGRAM_NUM_BUCKETS:-32768}"
export NGRAM_CHUNK_SIZE="${NGRAM_CHUNK_SIZE:-512}"
export NGRAM_ONLINE_CHUNK_TOKENS="${NGRAM_ONLINE_CHUNK_TOKENS:-131072}"
...
export PHRASE_ENABLED="${PHRASE_ENABLED:-0}"
...
export ARTIFACT_NGRAM_EXPORT="${ARTIFACT_NGRAM_EXPORT:-1}"
export ARTIFACT_NGRAM_MIN_ORDER="${ARTIFACT_NGRAM_MIN_ORDER:-2}"
export ARTIFACT_NGRAM_MAX_ORDER="${ARTIFACT_NGRAM_MAX_ORDER:-9}"
export ARTIFACT_NGRAM_BUCKETS="${ARTIFACT_NGRAM_BUCKETS:-32768}"
export ARTIFACT_GATE_STRENGTH="${ARTIFACT_GATE_STRENGTH:-1.0}"
...
echo "NGRAM: mode=${NGRAM_MODE} orders=${NGRAM_MIN_ORDER}-${NGRAM_MAX_ORDER} buckets=${NGRAM_NUM_BUCKETS} alpha=[${NGRAM_ALPHA_MIN},${NGRAM_ALPHA_MAX}] temp=${NGRAM_TEMPERATURE} leave_one_out=${NGRAM_LEAVE_ONE_OUT} chunk=${NGRAM_ONLINE_CHUNK_TOKENS}"
...
echo "ARTIFACT: export=${ARTIFACT_NGRAM_EXPORT} orders=${ARTIFACT_NGRAM_MIN_ORDER}-${ARTIFACT_NGRAM_MAX_ORDER} buckets=${ARTIFACT_NGRAM_BUCKETS} gate=${ARTIFACT_GATE_STRENGTH}"
