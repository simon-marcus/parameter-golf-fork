...
def eval_ngram_online(
        ...
        for ci in range(num_chunks):
            ...
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    if np.any(matched):
                        order_idx = (matched_order[matched] - ngram_cache.min_order).astype(np.int32)
                        centers = args.ngram_entropy_center - 0.25 * order_idx.astype(np.float32)
                        sig = 1.0 / (1.0 + np.exp(-args.ngram_entropy_scale * (entropy[matched] - centers)))
                        raw_alpha = args.ngram_alpha_min + (args.ngram_alpha_max - args.ngram_alpha_min) * sig
                        mults = _ORDER_MULTS[np.minimum(order_idx, len(_ORDER_MULTS) - 1)]
                    ...
                    if phrase_cache is not None:
                        ...
                        if np.any(phrase_matched):
                            len_floor = min(phrase_cache.probe_lengths)
                            len_ceil = max(phrase_cache.probe_lengths)
                            ...
                            alpha = args.phrase_alpha_min + (args.phrase_alpha_max - args.phrase_alpha_min) * len_frac
                    ...
                    scored_nll = torch.from_numpy(
                        -np.log(np.clip(p_blend, 1e-10, 1.0)).astype(np.float64)
    ...
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    log0(f"ngram_online:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} elapsed={time.perf_counter() - t0:.1f}s")
    return val_loss, val_bpb
