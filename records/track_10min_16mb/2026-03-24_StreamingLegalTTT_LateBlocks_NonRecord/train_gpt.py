...
class Hyperparameters:
    ...
    ttt_mode = os.environ.get("TTT_MODE", "chunk")
    ttt_param_mode = os.environ.get("TTT_PARAM_MODE", "freeze_first_n")
    ttt_last_n_blocks = int(os.environ.get("TTT_LAST_N_BLOCKS", 4))
    ttt_lr_schedule = os.environ.get("TTT_LR_SCHEDULE", "chunk_cosine")
...
def _ttt_bank_mask(base_model: GPT, param_name: str, keep_block_ids: set[int], device: torch.device) -> Tensor | None:
    n = base_model.num_layers
    if not keep_block_ids:
        return None
    if param_name in {"qo_bank", "kv_bank"}:
        mask = torch.zeros((2 * n, 1, 1), device=device, dtype=torch.float32)
        for bi in keep_block_ids:
            mask[bi] = 1.0
            mask[n + bi] = 1.0
        return mask
    if param_name in {"mlp_up_bank", "mlp_down_bank"}:
        mask = torch.zeros((n, 1, 1), device=device, dtype=torch.float32)
        for bi in keep_block_ids:
            mask[bi] = 1.0
        return mask
    return None
...
def _setup_ttt_params(
    args: Hyperparameters, base_model: GPT, device: torch.device, log0=print,
) -> tuple[list[Tensor], dict[int, Tensor], str]:
    num_layers = base_model.num_layers
    all_block_ids = set(range(num_layers))
    freeze_first_n = min(max(args.ttt_freeze_blocks, 0), num_layers)
    keep_last_n = min(max(args.ttt_last_n_blocks, 0), num_layers)
    keep_late_block_ids = set(range(num_layers - keep_last_n, num_layers))
    control_patterns = (
        "attn_scale", "mlp_scale", "resid_mix", "q_gain", "skip_weights", "smear",
        "ve_layer_scales", "ve_shared.scale", "attn_gate", "vr_lambda",
        "bigram.scale", "bigram.proj.weight", "ve_shared.proj.weight", "lm_head.weight",
    )
    ...
    if param_mode == "full":
        keep_block_ids = all_block_ids
        include_non_block = True
        control_only = False
        mode_desc = "full"
    elif param_mode == "freeze_first_n":
        keep_block_ids = set(range(freeze_first_n, num_layers))
        include_non_block = True
        control_only = False
        mode_desc = f"freeze_first_n:{freeze_first_n}"
    elif param_mode == "late_blocks":
        keep_block_ids = keep_late_block_ids
        include_non_block = False
        control_only = False
        mode_desc = f"late_blocks:last_{keep_last_n}"
    elif param_mode == "control_plus":
        keep_block_ids = set()
        include_non_block = False
        control_only = True
        mode_desc = "control_plus"
    else:
        raise ValueError(f"Unknown TTT_PARAM_MODE={param_mode}")

    for name, p in base_model.named_parameters():
        keep_param = False
        bank_mask = _ttt_bank_mask(base_model, name, keep_block_ids, device)
        if bank_mask is not None:
            if bank_mask.sum().item() > 0 and not control_only:
                keep_param = True
                grad_masks[id(p)] = bank_mask.to(dtype=p.dtype)
        elif name.startswith("blocks."):
            block_idx = int(name.split(".")[1])
            if block_idx in keep_block_ids and not control_only:
                keep_param = True
        elif control_only:
            keep_param = any(pattern in name for pattern in control_patterns)
        elif include_non_block:
            keep_param = True

        p.requires_grad_(keep_param)
        if keep_param:
            ttt_params.append(p)

    unfrozen_params = sum(p.numel() for p in ttt_params)
    frozen_params = sum(p.numel() for p in base_model.parameters() if not p.requires_grad)
    log0(f"ttt_setup:param_mode={mode_desc} unfrozen={unfrozen_params} frozen={frozen_params}")
    return ttt_params, grad_masks, mode_desc


def _ttt_set_lr(args: Hyperparameters, optimizer: torch.optim.Optimizer, progress: float) -> float:
    schedule = args.ttt_lr_schedule
    if schedule == "constant":
        lr = args.ttt_lr
    elif schedule in {"cosine", "token_cosine", "chunk_cosine"}:
        lr = args.ttt_lr * 0.5 * (1.0 + math.cos(math.pi * min(max(progress, 0.0), 1.0)))
    else:
        raise ValueError(f"Unknown TTT_LR_SCHEDULE={schedule}")
    for pg in optimizer.param_groups:
        pg["lr"] = lr
    return lr


def _ttt_reduce_and_mask_grads(ttt_params: list[Tensor], grad_masks: dict[int, Tensor], world_size: int) -> None:
    if world_size > 1:
        for p in ttt_params:
...
def _ttt_accumulate_metrics(
    loss_sum: Tensor, token_count: Tensor, byte_count: Tensor, scored_nll: Tensor, tgt: Tensor, prev: Tensor,
    base_bytes_lut: Tensor, has_leading_space_lut: Tensor, is_boundary_token_lut: Tensor,
) -> int:
    scored_detached = scored_nll.detach().to(torch.float64)
    loss_sum += scored_detached.sum()
    tokens = int(scored_detached.numel())
...
def eval_val_sliding_ttt(
    ...
    """Legal TTT over the eval token stream.

    chunk mode:
      score a whole chunk, then train on the already-scored chunk
    stream mode:
      score the newly revealed suffix of each window, then immediately train on that same suffix
    """
    ...
    ttt_params, grad_masks, mode_desc = _setup_ttt_params(args, base_model, device, log0=log0)
    optimizer = torch.optim.SGD(ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
    adapted_token_count = torch.zeros((), device=device, dtype=torch.float64)
    scored_token_count = torch.zeros((), device=device, dtype=torch.float64)
    base_model.eval()
    t0 = time.perf_counter()

    log0(
        f"ttt:start mode={args.ttt_mode} param_mode={mode_desc} chunks={num_chunks} "
        f"chunk_tokens={ttt_chunk} total_windows={len(window_starts)} stride={stride} "
        f"ttt_lr={args.ttt_lr} ttt_epochs={args.ttt_epochs}"
    )

    if args.ttt_mode == "chunk":
        for ci in range(num_chunks):
            ...
            with torch.inference_mode():
                for bi in range(0, len(my_windows), batch_seqs):
                    ...
                    for i, ws in enumerate(batch_ws):
                        wlen = wlens[i]
                        s = 0 if ws == 0 else max(wlen - stride, 0)
                        tokens = _ttt_accumulate_metrics(
                            loss_sum, token_count, byte_count,
                            nll[i, s:wlen], y_batch[i, s:wlen], x_batch[i, s:wlen],
            ...
            if not is_last_chunk and args.ttt_epochs > 0:
                ...
                if chunk_seqs > 0:
                    _ttt_set_lr(args, optimizer, ci / max(num_chunks - 1, 1))
                    ...
                    for _ep in range(args.ttt_epochs):
                        for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                            ...
                            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                                loss = base_model(x, y)
                            loss.backward()
                            _ttt_reduce_and_mask_grads(ttt_params, grad_masks, world_size)
                            torch.nn.utils.clip_grad_norm_(ttt_params, args.ttt_grad_clip)
                            optimizer.step()
                            adapted_token_count += float(y.numel())
    ...
    elif args.ttt_mode == "stream":
        my_s = (len(window_starts) * rank) // world_size
        my_e = (len(window_starts) * (rank + 1)) // world_size
        my_windows = window_starts[my_s:my_e]
        local_total_windows = len(my_windows)
        for bi in range(0, local_total_windows, batch_seqs):
            batch_ws = my_windows[bi:bi + batch_seqs]
            bsz = len(batch_ws)
            x_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
            y_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
            ...
            for i, ws in enumerate(batch_ws):
                ...
                wlen = end - ws
                wlens.append(wlen)
                starts.append(0 if ws == 0 else max(wlen - stride, 0))
                chunk_tok = val_tokens[ws:end + 1].to(dtype=torch.int64, device=device)
                x_batch[i, :wlen] = chunk_tok[:-1]
                y_batch[i, :wlen] = chunk_tok[1:]

            optimizer.zero_grad(set_to_none=True)
            progress = 0.0 if local_total_windows <= 1 else bi / max(local_total_windows - 1, 1)
            lr = _ttt_set_lr(args, optimizer, progress)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = base_model.forward_logits(x_batch)
            nll = F.cross_entropy(
                logits.reshape(-1, logits.size(-1)).float(),
            ...
            for i, ws in enumerate(batch_ws):
                wlen = wlens[i]
                s = starts[i]
                scored_slice = nll[i, s:wlen]
                tokens = _ttt_accumulate_metrics(
                    loss_sum, token_count, byte_count,
                    scored_slice, y_batch[i, s:wlen], x_batch[i, s:wlen],
                    base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                )
                scored_token_count += tokens
                adapt_tokens += tokens
                scored_sum = scored_slice.sum()
                adapt_loss_sum = scored_sum if adapt_loss_sum is None else adapt_loss_sum + scored_sum

            if adapt_tokens > 0:
                adapt_loss = adapt_loss_sum / adapt_tokens
                adapt_loss.backward()
                _ttt_reduce_and_mask_grads(ttt_params, grad_masks, world_size)
                torch.nn.utils.clip_grad_norm_(ttt_params, args.ttt_grad_clip)
                optimizer.step()
                adapted_token_count += float(adapt_tokens)

            global_window_idx = my_s + bi + len(batch_ws)
            if rank == 0 and (global_window_idx % 128 == 0 or global_window_idx >= len(window_starts)):
                elapsed = time.perf_counter() - t0
                rl = loss_sum.item() / max(token_count.item(), 1)
                rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1)) if token_count.item() > 0 else 0.0
                log0(
                    f"  ttt_stream windows:{global_window_idx}/{len(window_starts)} "
                    f"scored_tokens:{int(scored_token_count.item())} adapted_tokens:{int(adapted_token_count.item())} "
                    f"lr:{lr:.6g} bpb={rbpb:.6f} time={elapsed:.1f}s"
                )
    else:
        raise ValueError(f"Unknown TTT_MODE={args.ttt_mode}")

    if dist.is_available() and dist.is_initialized():
        for t in (loss_sum, token_count, byte_count, adapted_token_count, scored_token_count):
            dist.all_reduce(t, op=dist.ReduceOp.SUM)

    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    assert adapted_token_count.item() <= scored_token_count.item() + 1e-6
    for p in base_model.parameters():
        p.requires_grad_(True)
    base_model.eval()
    log0(
        f"ttt:done mode={args.ttt_mode} val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
        f"scored_tokens={int(scored_token_count.item())} adapted_tokens={int(adapted_token_count.item())} "
