...
class Hyperparameters:
    ...
    ngram_num_buckets = int(os.environ.get("NGRAM_NUM_BUCKETS", 8_388_608))  # 8M
    ...
    ngram_mode = os.environ.get("NGRAM_MODE", "online")  # "online" or "two_pass"
    ngram_online_chunk_tokens = int(os.environ.get("NGRAM_ONLINE_CHUNK_TOKENS", 8192))
    ...
    ngram_calibration_min_tokens = int(os.environ.get("NGRAM_CALIBRATION_MIN_TOKENS", 131072))
    ngram_calibration_max_tokens = int(os.environ.get("NGRAM_CALIBRATION_MAX_TOKENS", 524288))
...
class PhraseCache:
    """Separate per-length phrase tables to avoid cross-length contamination."""
    ...
    def update_range(self, tokens_np: np.ndarray, start: int, end: int):
        for length in self.probe_lengths:
            ctx_h, full_h, _ = self._compute_hashes(tokens_np, start, end, length)
            if ctx_h is None:
                continue
            self._bincount_add(self.ctx_tables[length], ctx_h)
            self._bincount_add(self.full_tables[length], full_h)

    def score_positions(self, tokens_np: np.ndarray, positions: np.ndarray, min_count: int = 1):
        N = len(positions)
...
def _calibrate_online_cache_blend(
    args: Hyperparameters,
    calib_data: dict[str, list[np.ndarray]],
    phrase_probe_lengths: list[int],
    log0=print,
) -> tuple[float, float, float]:
    if not calib_data["p_model"]:
        return args.ngram_alpha_max, args.ngram_entropy_center, args.phrase_alpha_max

    p_model = np.concatenate(calib_data["p_model"]).astype(np.float32, copy=False)
    entropy = np.concatenate(calib_data["entropy"]).astype(np.float32, copy=False)
    ngram_prob = np.concatenate(calib_data["ngram_prob"]).astype(np.float32, copy=False)
    matched_order = np.concatenate(calib_data["matched_order"]).astype(np.int32, copy=False)
    phrase_prob = None
    matched_len = None
    if calib_data["phrase_prob"]:
        phrase_prob = np.concatenate(calib_data["phrase_prob"]).astype(np.float32, copy=False)
        matched_len = np.concatenate(calib_data["matched_len"]).astype(np.int32, copy=False)

    alpha_grid = _parse_float_grid(
        args.ngram_calibration_alpha_max_grid,
        (args.ngram_alpha_max,),
    )
    center_grid = _parse_float_grid(
        args.ngram_calibration_entropy_center_grid,
        (args.ngram_entropy_center,),
    )
    phrase_alpha_grid = _parse_float_grid(
        args.phrase_calibration_alpha_max_grid,
    ...
    for alpha_max in alpha_grid:
        for center in center_grid:
            for phrase_alpha_max in phrase_alpha_grid:
                p_blend = _blend_online_cache_probs(
                    args,
                    p_model,
                    entropy,
                    ngram_prob,
    ...
    log0(
        f"ngram_online:calibration tokens={len(p_model)} "
...
def eval_ngram_online(
    args: Hyperparameters, base_model: nn.Module, rank: int, world_size: int,
    device: torch.device, val_tokens: Tensor, base_bytes_lut: Tensor,
    has_leading_space_lut: Tensor, is_boundary_token_lut: Tensor,
    stride: int, batch_seqs: int = 32, log0=print,
) -> tuple[float, float]:
    """Score-first online n-gram + phrase cache evaluation."""
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    tokens_np = val_tokens.cpu().numpy().astype(np.int16, copy=False)
    online_chunk = max(args.ngram_online_chunk_tokens, stride)
    probe_lengths = [
        int(part.strip()) for part in args.phrase_probe_lengths.split(",") if part.strip()
    ]
    ...
    ) if args.phrase_enabled and probe_lengths else None
    ...
    calibration_done = not args.ngram_calibration_enabled
    ...
    log0(
        f"ngram_online:start chunks={num_chunks} chunk_tokens={online_chunk} "
        f"orders={args.ngram_min_order}-{args.ngram_max_order} "
        f"phrase_lengths={probe_lengths if phrase_cache is not None else []} "
        f"temp={args.ngram_temperature:.3f}"
    )
        ...
        for ci in range(num_chunks):
            ...
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    if not calibration_done and calibration_tokens < args.ngram_calibration_max_tokens:
                        remaining = args.ngram_calibration_max_tokens - calibration_tokens
                        take = min(seg_len, remaining)
                        if take > 0:
                            calib_data["p_model"].append(p_model[:take].copy())
                            calib_data["entropy"].append(entropy[:take].copy())
                            calib_data["ngram_prob"].append(ngram_prob[:take].copy())
                            calib_data["matched_order"].append(matched_order[:take].copy())
                            if phrase_prob is not None and matched_len is not None:
                                calib_data["phrase_prob"].append(phrase_prob[:take].copy())
                                calib_data["matched_len"].append(matched_len[:take].copy())
                            calibration_tokens += take
                    ...
                    scored_nll = torch.from_numpy(
                        -np.log(p_blend).astype(np.float64)
                    ...
                    if (
                        ...
                        and calibration_tokens >= args.ngram_calibration_min_tokens
                    ):
                        (
                            active_ngram_alpha_max,
                            active_ngram_entropy_center,
                            active_phrase_alpha_max,
                        ) = _calibrate_online_cache_blend(
                            args,
                            calib_data,
                            probe_lengths,
                            log0=log0,
            ...
            ngram_cache.update_range(tokens_np, chunk_start, chunk_end + 1)
            if phrase_cache is not None:
                phrase_cache.update_range(tokens_np, chunk_start, chunk_end + 1)

            if rank == 0 and (ci % 10 == 0 or ci == num_chunks - 1):
                elapsed = time.perf_counter() - t0
                rl = loss_sum.item() / max(token_count.item(), 1.0)
                rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1.0))
                log0(f"  ngram_online [{ci+1}/{num_chunks}] bpb={rbpb:.6f} time={elapsed:.1f}s")
    ...
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    log0(
        f"ngram_online:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
        f"alpha_max={active_ngram_alpha_max:.3f} entropy_center={active_ngram_entropy_center:.3f} "
        f"phrase_alpha_max={active_phrase_alpha_max:.3f} elapsed={time.perf_counter() - t0:.1f}s"
    )
    return val_loss, val_bpb
...
def main() -> None:
    ...
    if args.ngram_enabled:
        ...
        if args.ngram_mode == "online":
            ng_val_loss, ng_val_bpb = eval_ngram_online(
                args, ngram_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                stride=args.eval_stride, log0=log0,
            )
            mode_name = "ngram_online"
        else:
