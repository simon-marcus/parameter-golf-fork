...
class Hyperparameters:
    ...
    ttt_use_prequant = bool(int(os.environ.get("TTT_USE_PREQUANT", "0")))
...
def eval_val_sliding_ttt(
    ...
    for name, p in base_model.named_parameters():
        ...
        # Bank parameters: mask frozen block slices via gradient mask
        if name in ("qo_bank", "kv_bank"):
            # qo_bank/kv_bank shape: (2*num_layers, dim1, dim2)
            # Slices [i] = block i Q/K, slices [num_layers+i] = block i O/V
            ...
            # mlp banks shape: (num_layers, dim1, dim2)
...
def main() -> None:
    ...
    if args.ttt_enabled:
        ...
        ttt_model = base_model if args.ttt_use_prequant else eval_model
        log0(f"legal_ttt_mode:{'prequant' if args.ttt_use_prequant else 'postquant'}")
