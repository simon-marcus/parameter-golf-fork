...
class Hyperparameters:
    ...
    jepa_loss_weight = float(os.environ.get("JEPA_LOSS_WEIGHT", 0.10))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.jepa_loss_weight = float(os.environ.get("JEPA_LOSS_WEIGHT", 0.10))
        ...
        # Register the JEPA predictor under blocks[0] so it follows the native
        # optimizer split without a separate parameter group.
        self.blocks[0].jepa_in = CastedLinear(model_dim, model_dim, bias=False)
        self.blocks[0].jepa_out = CastedLinear(model_dim, model_dim, bias=False)
    ...
    def predict_patch_latents(self, patch_states: Tensor) -> Tensor:
        ...
        x = torch.relu(self.blocks[0].jepa_in(x))
        return self.blocks[0].jepa_out(x.square())
...
def main() -> None:
    ...
    # For the JEPA submission we include embed/other tensors in the int6 bucket
    # to keep the final artifact under the 16 MB cap without changing the trained
    # weights or using any data-dependent post-training calibration.
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn", "embed", "other"})
