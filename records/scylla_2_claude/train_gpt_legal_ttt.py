...
class Hyperparameters:
    ...
    embed_wd = float(os.environ.get("EMBED_WD", str(muon_wd)))
    ...
    ttt_chunk_bytes = int(os.environ.get("TTT_CHUNK_BYTES", "0"))
    ttt_freeze_blocks = int(os.environ.get("TTT_FREEZE_BLOCKS", 2))
    ttt_freeze_embeddings = bool(int(os.environ.get("TTT_FREEZE_EMBEDDINGS", "0")))
    ttt_momentum = float(os.environ.get("TTT_MOMENTUM", 0.9))
    ttt_batch_seqs = int(os.environ.get("TTT_BATCH_SEQS", 32))
    ttt_grad_clip = float(os.environ.get("TTT_GRAD_CLIP", 1.0))
    ttt_mode = os.environ.get("TTT_MODE", "classic")
    ttt_adapter_rank = int(os.environ.get("TTT_ADAPTER_RANK", 32))
    ttt_adapter_last_n = int(os.environ.get("TTT_ADAPTER_LAST_N", 4))
    ttt_adapter_scale = float(os.environ.get("TTT_ADAPTER_SCALE", 1.0))
    # N-gram eval cache
    ...
    causal_cache_enabled = bool(int(os.environ.get("CAUSAL_CACHE_ENABLED", "0")))
    causal_cache_min_order = int(os.environ.get("CAUSAL_CACHE_MIN_ORDER", 2))
    causal_cache_max_order = int(os.environ.get("CAUSAL_CACHE_MAX_ORDER", 4))
    causal_cache_min_count = int(os.environ.get("CAUSAL_CACHE_MIN_COUNT", 2))
    causal_cache_alpha_min = float(os.environ.get("CAUSAL_CACHE_ALPHA_MIN", 0.05))
    causal_cache_alpha_max = float(os.environ.get("CAUSAL_CACHE_ALPHA_MAX", 0.25))
    causal_cache_entropy_center = float(os.environ.get("CAUSAL_CACHE_ENTROPY_CENTER", 2.5))
    causal_cache_entropy_scale = float(os.environ.get("CAUSAL_CACHE_ENTROPY_SCALE", 1.5))
    ...
    # Export / compression
    compressor = os.environ.get("COMPRESSOR", "lzma").lower()
    byte_shuffle = bool(int(os.environ.get("BYTE_SHUFFLE", "0")))
    matrix_bits = int(os.environ.get("MATRIX_BITS", 6))
    embed_bits = int(os.environ.get("EMBED_BITS", 8))
    matrix_clip_mode = os.environ.get("MATRIX_CLIP_MODE", "quantile").lower()
    embed_clip_mode = os.environ.get("EMBED_CLIP_MODE", "quantile").lower()
...
class GPT(nn.Module):
    def __init__(
        ...
        self.encoder_indices = self.base_encoder_indices
        self.decoder_indices = self.base_decoder_indices
        if num_loops > 0:
            loop_start = max(0, min(loop_start, num_layers - 1))
            loop_end = max(loop_start, min(loop_end, num_layers - 1))
        ...
        self.max_skip_weights = max(
            min(len(self.base_encoder_indices), len(self.base_decoder_indices)),
        ...
        self._init_weights()
    def _active_indices(self) -> tuple[list[int], list[int]]:
        if self.looping_active:
            return self.encoder_indices, self.decoder_indices
        return self.base_encoder_indices, self.base_decoder_indices
    def forward_features(self, input_ids: Tensor, adapter_bank: nn.Module | None = None) -> Tensor:
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        if self.smear_enabled:
            x = self.smear(x)
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        encoder_indices, decoder_indices = self._active_indices()
        num_skip_weights = min(len(encoder_indices), len(decoder_indices), self.skip_weights.shape[0])
        for bi in encoder_indices:
            ...
            if adapter_bank is not None:
                x = adapter_bank.apply_layer(bi, x)
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        for i, bi in enumerate(decoder_indices):
            if skips and i < num_skip_weights:
...
class ResidualAdapterBank(nn.Module):
    """Small residual adapters for eval-time TTT without touching base weights."""

    def __init__(self, num_layers: int, model_dim: int, rank: int, selected_layers: list[int], scale: float = 1.0):
        super().__init__()
        self.selected_layers = sorted(set(selected_layers))
        self.layer_to_slot = {layer_idx: slot for slot, layer_idx in enumerate(self.selected_layers)}
        self.down = nn.ParameterList()
        self.up = nn.ParameterList()
        self.scale = nn.ParameterList()
        for _layer_idx in self.selected_layers:
            down = nn.Parameter(torch.empty(rank, model_dim, dtype=torch.float32))
            up = nn.Parameter(torch.zeros(model_dim, rank, dtype=torch.float32))
            scale_param = nn.Parameter(torch.tensor(scale, dtype=torch.float32))
            nn.init.normal_(down, std=0.02)
            self.down.append(down)
            self.up.append(up)
            self.scale.append(scale_param)

    def apply_layer(self, layer_idx: int, x: Tensor) -> Tensor:
        slot = self.layer_to_slot.get(layer_idx)
        if slot is None:
            return x
        normed = F.rms_norm(x, (x.size(-1),))
        hidden = F.linear(normed, self.down[slot].to(dtype=normed.dtype))
        hidden = F.silu(hidden)
        delta = F.linear(hidden, self.up[slot].to(dtype=hidden.dtype))
        return x + self.scale[slot].to(dtype=delta.dtype) * delta


class AdapterizedTTTWrapper(nn.Module):
    def __init__(self, base_model: GPT, adapter_rank: int, adapter_last_n: int, adapter_scale: float):
        super().__init__()
        self.base_model = base_model
        num_layers = len(base_model.blocks)
        if adapter_last_n <= 0 or adapter_last_n >= num_layers:
            selected_layers = list(range(num_layers))
        else:
            selected_layers = list(range(num_layers - adapter_last_n, num_layers))
        self.adapters = ResidualAdapterBank(num_layers, base_model.tok_emb.embedding_dim, adapter_rank, selected_layers, adapter_scale)

    def forward_logits(self, input_ids: Tensor) -> Tensor:
        x = self.base_model.forward_features(input_ids, adapter_bank=self.adapters)
        return self.base_model.project_logits(x)
...
def _ttt_chunk_bounds(
    ...
    if args.ttt_chunk_bytes <= 0:
        if args.ttt_chunk_tokens <= 0:
            raise ValueError("TTT_CHUNK_TOKENS must be positive when TTT_CHUNK_BYTES is unset")
        bounds = [0]
        while bounds[-1] < total_tokens:
    ...
    prefix[1:] = torch.cumsum(token_bytes.cpu(), dim=0)

    bounds = [0]
    start = 0
    while start < total_tokens:
        target_bytes = int(prefix[start].item()) + args.ttt_chunk_bytes
        needle = torch.tensor(target_bytes, dtype=prefix.dtype)
        end = int(torch.searchsorted(prefix, needle, right=False).item())
...
def eval_val_sliding_ttt(
    ...
    total_tokens = val_tokens.numel() - 1
    ttt_mode = args.ttt_mode.lower()
    chunk_bounds = _ttt_chunk_bounds(
        args,
        total_tokens,
        val_tokens,
        base_bytes_lut,
    ...
    num_chunks = max(len(chunk_bounds) - 1, 0)
    chunk_stops = torch.tensor(chunk_bounds[1:], dtype=torch.int64)
    ...
    for ws in window_starts:
        ...
        s = 0 if ws == 0 else max(wlen - stride, 0)
        scored_start = ws + s
        ci = min(int(torch.searchsorted(chunk_stops, torch.tensor(scored_start, dtype=torch.int64), right=True).item()), num_chunks - 1)
        chunk_windows[ci].append(ws)

    chunk_desc = f"chunk_bytes={args.ttt_chunk_bytes}" if args.ttt_chunk_bytes > 0 else f"chunk_tokens={args.ttt_chunk_tokens}"
    log0(f"ttt_sliding:start chunks={num_chunks} {chunk_desc} "
         f"total_windows={len(window_starts)} stride={stride} "
         f"ttt_mode={ttt_mode} ttt_lr={args.ttt_lr} ttt_epochs={args.ttt_epochs} "
         f"freeze_blocks={args.ttt_freeze_blocks} freeze_embeddings={args.ttt_freeze_embeddings} "
         f"adapter_rank={args.ttt_adapter_rank} adapter_last_n={args.ttt_adapter_last_n}")
    ...
    frozen_block_ids: set[int] = set()
    freeze_emb = args.ttt_freeze_embeddings

    if ttt_mode == "adapter":
        for p in base_model.parameters():
            p.requires_grad_(False)
        ttt_model = AdapterizedTTTWrapper(
            base_model=base_model,
            adapter_rank=args.ttt_adapter_rank,
            adapter_last_n=args.ttt_adapter_last_n,
            adapter_scale=args.ttt_adapter_scale,
        ).to(device=device)
        ttt_model.train()
        ttt_params = list(ttt_model.adapters.parameters())
        adapter_layers = sorted(ttt_model.adapters.selected_layers)
        log0(f"ttt_sliding:adapter selected_layers={adapter_layers} adapter_params={sum(p.numel() for p in ttt_params)}")
    else:
        # --- Parameter freeze logic (byte-native aware) ---
        frozen_block_ids = set(range(min(args.ttt_freeze_blocks, num_layers)))
        unfrozen_block_ids = set(range(num_layers)) - frozen_block_ids

        # Non-block, non-bank parameter names that count as "embedding/output path"
        _EMB_PREFIXES = ("tok_emb.", "lm_head.", "bigram.", "smear.", "ve_shared.",
                         "ve_layer_scales.", "final_norm.", "skip_weights", "mtp_heads.")

        for name, p in base_model.named_parameters():
            keep = True

            if name in ("qo_bank", "kv_bank"):
                mask = torch.zeros(2 * num_layers, 1, 1, device=device, dtype=p.dtype)
                for bi in unfrozen_block_ids:
                    mask[bi] = 1.0
                    mask[num_layers + bi] = 1.0
                if mask.sum().item() == 0:
                    keep = False
                else:
                    grad_masks[id(p)] = mask
            elif name in ("mlp_up_bank", "mlp_down_bank"):
                mask = torch.zeros(num_layers, 1, 1, device=device, dtype=p.dtype)
                for bi in unfrozen_block_ids:
                    mask[bi] = 1.0
                if mask.sum().item() == 0:
                    keep = False
                ...
                if block_idx in frozen_block_ids:
                    keep = False
            elif any(name.startswith(pfx) or name == pfx.rstrip(".") for pfx in _EMB_PREFIXES):
                if freeze_emb:
                    keep = False
        ...
        log0(f"ttt_sliding:params unfrozen={unfrozen_count} frozen={frozen_count} "
             f"frozen_blocks={sorted(frozen_block_ids)} freeze_emb={freeze_emb}")
    ...
    for ci in range(num_chunks):
        ...
        if not windows:
            continue
        chunk_start = chunk_bounds[ci]
        chunk_end = chunk_bounds[ci + 1]
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    logits = ttt_model.forward_logits(x_batch)
                nll = F.cross_entropy(
                    logits.reshape(-1, logits.size(-1)).float(),
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        # Apply bank gradient masks (zero out frozen block slices)
                        for p in ttt_params:
...
class ExactCausalTokenCache:
    """Exact causal cache with normalized next-token distributions."""

    def __init__(self, min_order: int, max_order: int):
        self.min_order = min_order
        self.max_order = max_order
        self.tables: list[dict[tuple[int, ...], dict[int, int]]] = [dict() for _ in range(max_order - min_order + 1)]

    def _context(self, tokens_np: np.ndarray, pos: int, order: int) -> tuple[int, ...] | None:
        ctx_len = order - 1
        if pos < ctx_len:
            return None
        start = pos - ctx_len
        return tuple(int(x) for x in tokens_np[start:pos])

    def update(self, tokens_np: np.ndarray, pos: int):
        target = int(tokens_np[pos])
        for order in range(self.min_order, self.max_order + 1):
            ctx = self._context(tokens_np, pos, order)
            if ctx is None:
                continue
            table = self.tables[order - self.min_order]
    ...
    def score_target(self, tokens_np: np.ndarray, pos: int, target: int, min_count: int) -> tuple[float, int, int]:
        for order in range(self.max_order, self.min_order - 1, -1):
            ...
            if total < min_count:
                continue
            return counts.get(target, 0) / total, order, total
        return 0.0, -1, 0


def eval_val_sliding_causal_cache(
    args: Hyperparameters,
    base_model: nn.Module,
    rank: int,
    world_size: int,
    ...
    """One-pass exact causal cache with normalized interpolation.

    Each scored token is evaluated once, blended using only a cache built from
    earlier scored tokens, then committed to the cache after scoring.
    """
    if world_size != 1:
        raise RuntimeError("CAUSAL_CACHE_ENABLED currently supports only WORLD_SIZE=1 for strict causal ordering")
    ...
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= 1]
    cache = ExactCausalTokenCache(args.causal_cache_min_order, args.causal_cache_max_order)
        ...
        for bi in range(0, len(window_starts), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                ...
                for j in range(s, wlen):
                    target = int(y_batch[i, j].item())
                    prev = int(x_batch[i, j].item())
                    pos = ws + j + 1  # target token position in val_tokens
                    model_prob = float(probs[i, j, target].item())
                    cache_prob, matched_order, total = cache.score_target(
                        tokens_np, pos, target, min_count=args.causal_cache_min_count
                    )
                    if matched_order >= 0:
                        ent = float(entropy[i, j].item())
                        sig = 1.0 / (1.0 + math.exp(-args.causal_cache_entropy_scale * (ent - args.causal_cache_entropy_center)))
                        alpha = args.causal_cache_alpha_min + (args.causal_cache_alpha_max - args.causal_cache_alpha_min) * sig
                        blend_prob = (1.0 - alpha) * model_prob + alpha * cache_prob
                        matched += 1
                        used_alpha += alpha
                    else:
                        blend_prob = model_prob
                    blend_prob = max(blend_prob, 1e-10)
                    loss_sum += -math.log(blend_prob)
                    token_count += 1.0
                    byte_val = float(base_bytes_lut[target].item())
                    if bool(has_leading_space_lut[target].item()) and not bool(is_boundary_token_lut[prev].item()):
                        byte_val += 1.0
                    byte_count += byte_val
                    cache.update(tokens_np, pos)
            if bi == 0 or (bi // max(batch_seqs, 1)) % 10 == 0 or bi + batch_seqs >= len(window_starts):
                cur_loss = loss_sum / max(token_count, 1.0)
                cur_bpb = cur_loss / math.log(2.0) * (token_count / max(byte_count, 1.0))
                log0(f"causal_cache:window_batch {min(bi + batch_seqs, len(window_starts))}/{len(window_starts)} "
                     f"matched={matched} running_bpb={cur_bpb:.6f}")

    val_loss = loss_sum / token_count
    val_bpb = val_loss / math.log(2.0) * (token_count / byte_count)
    mean_alpha = used_alpha / matched if matched > 0 else 0.0
    log0(f"causal_cache:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
         f"matched={matched}/{int(token_count)} mean_alpha={mean_alpha:.4f} "
         f"elapsed={time.perf_counter() - t0:.1f}s")
    base_model.train()
    return val_loss, val_bpb
    ...
    *,
    ...
    if t32.ndim == 2:
        if clip_mode == "std":
            ...
            s = (row_std * clip_sigmas / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
    ...
    if clip_mode == "std":
        ...
        scale = torch.tensor(max(std * clip_sigmas / clip_range, 1.0 / clip_range), dtype=torch.float16)
...
def mixed_quantize_export(state_dict: dict[str, Tensor], args: Hyperparameters):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if cat in {"embed", "mlp", "attn"} and t.ndim >= 1:
            bits = args.embed_bits if cat == "embed" else args.matrix_bits
            clip_mode = args.embed_clip_mode if cat == "embed" else args.matrix_clip_mode
            clip_sigmas = args.embed_clip_sigmas if cat == "embed" else args.matrix_clip_sigmas
            q, s = quantize_intx_per_row(t, bits=bits, clip_mode=clip_mode, clip_sigmas=clip_sigmas)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def _compress_blob(data: bytes, args: Hyperparameters) -> bytes:
    raw = _byte_shuffle(data) if args.byte_shuffle else data
    if args.compressor == "lzma":
        return lzma.compress(raw, preset=6)
    if args.compressor == "zlib":
...
def _decompress_blob(data: bytes, args: Hyperparameters) -> bytes:
    if args.compressor == "lzma":
...
def main() -> None:
    ...
    log0(
        f"smear_enabled:{args.smear_enabled} loops:{args.num_loops} "
        f"loop_range:{args.loop_start}-{args.loop_end} enable_looping_at:{args.enable_looping_at:.3f}"
    )
    if args.num_loops > 0:
        log0(f"loop_indices:encoder={base_model.encoder_indices} decoder={base_model.decoder_indices}")
    log0(
    ...
    log0(
        f"export:compressor={args.compressor} byte_shuffle={int(args.byte_shuffle)} "
        f"matrix_bits={args.matrix_bits} embed_bits={args.embed_bits} "
        f"matrix_clip_mode={args.matrix_clip_mode} embed_clip_mode={args.embed_clip_mode}"
    )
    log0(
