...
set -euo pipefail
#
# Scylla-v2 byte-native TTT sweep — 6 configurations on 1xH100 smoke.
#
# Usage:
#   # Run a single config:
#   bash launch_sweep.sh A1
#
#   # Run all configs sequentially:
#   bash launch_sweep.sh ALL
#
#   # Run on pod (after syncing records/scylla_2_claude/ to the pod):
#   ssh root@HOST -p PORT "cd /workspace/parameter-golf && bash records/scylla_2_claude/launch_sweep.sh A1"

DATA_ROOT_MODE="${DATA_ROOT_MODE:-workspace}"
NPROC_PER_NODE="${NPROC_PER_NODE:-1}"
USE_COMPILE="${USE_COMPILE:-0}"
SCREEN_SECONDS="${SCREEN_SECONDS:-2400}"  # 40 min budget per smoke run

cd /workspace/parameter-golf

VERIFY_SCRIPT="/workspace/parameter-golf/verify_runpod_data_ready.sh"
SCRIPT_PATH="/workspace/parameter-golf/records/scylla_2_claude/train_gpt_legal_ttt.py"
SWEEP_ROOT="/workspace/parameter-golf/records/scylla_2_claude/runs"
...
# --- Shared config (Scylla-v2 model defaults matching the 8x record run) ---
...
# Format: NAME CHUNK_TOKENS LR EPOCHS FREEZE_BLOCKS FREEZE_EMBEDDINGS
#
# A1: Byte-equivalent chunk size (2x current), baseline freeze
# A2: Richer chunks + fewer epochs
# A3: Late-layer-only adaptation (freeze 6 of 11 blocks)
# A4: Late-layer + frozen embeddings (key byte-native hypothesis)
# A5: Even larger chunks + full byte-native freeze
# A6: Current best Scylla config (control)

declare -A CONFIGS
CONFIGS[A1]="65536 0.0015 3 2 0"
CONFIGS[A2]="65536 0.001  2 2 0"
CONFIGS[A3]="65536 0.001  2 6 0"
CONFIGS[A4]="65536 0.001  2 6 1"
CONFIGS[A5]="96000 0.001  2 6 1"
CONFIGS[A6]="34304 0.0015 3 2 0"

run_config() {
  local NAME="$1"
  local CFG="${CONFIGS[$NAME]}"
  read -r CHUNK LR EPOCHS FREEZE_BLOCKS FREEZE_EMB <<< "$CFG"
  ...
  export RUN_ID="scylla_ttt_sweep_${NAME}"

  local OUT_DIR="$SWEEP_ROOT/$NAME"
  mkdir -p "$OUT_DIR"

  echo "=== $NAME: chunk=$CHUNK lr=$LR epochs=$EPOCHS freeze_blocks=$FREEZE_BLOCKS freeze_emb=$FREEZE_EMB ==="
  echo "  output: $OUT_DIR/train.log"

  # Save config for later analysis
...
if [[ "$TARGET" == "ALL" ]]; then
  for NAME in A6 A1 A2 A3 A4 A5; do
