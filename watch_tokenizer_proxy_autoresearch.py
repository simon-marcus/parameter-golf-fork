#!/usr/bin/env python3
"""Pretty terminal watcher for proxy-calibrated tokenizer autoresearch."""
...
DEFAULT_DIR = ROOT / "autoresearch" / "tokenizer_proxy_discovery"
...
def format_params(row: dict) -> str:
    ...
    return (
        f"v={params.get('vocab_size', '-')}"
        f" m={params.get('model_type', '-')}"
...
def render_dashboard(base_dir: Path, *, tail_count: int, history_count: int) -> str:
    ...
    calibration = read_json(base_dir / "proxy_calibration.json") or {}
    ...
    lines: list[str] = []
    lines.append("Proxy-Calibrated Tokenizer Autoresearch")
    lines.append("=" * 80)
    lines.append(
    ...
    lines.append(
        ...
        f" | Best artifact: {format_pct(best_metrics.get('estimated_artifact_budget_frac'))}"
    )
    lines.append(
    ...
    lines.append(
        ...
        f" | Rows: {calibration.get('rows_used', '-')}"
        f" | Baseline BPB: {format_float(calibration.get('baseline_postquant_val_bpb'))}"
    )
    if weights:
        labels = ["bias", "tpb_delta", "dead_delta", "artifact_delta", "is_tm"]
        lines.append("Weights: " + "  ".join(f"{l}={format_float(w, 4)}" for l, w in zip(labels, weights)))
    lines.append("")
    ...
    if latest is None:
        ...
        lines.append(
            ...
            f" | family {latest_metrics.get('family', '-')}"
        )
        lines.append(
            f"tpb {format_float(latest_metrics.get('tokens_per_byte'))}"
            f" | dead {format_pct(latest_metrics.get('dead_vocab_frac'))}"
            f" | artifact {format_pct(latest_metrics.get('estimated_artifact_budget_frac'))}"
            f" | vocab {latest_metrics.get('vocab_size', '-')}"
        )
    ...
    if not history:
        ...
        for row in history[-history_count:]:
            ...
            lines.append(
                ...
                f"art={format_pct(metrics.get('estimated_artifact_budget_frac'))} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pretty watcher for proxy-calibrated tokenizer autoresearch.")
    parser.add_argument("--dir", type=Path, default=DEFAULT_DIR, help="Autoresearch tokenizer proxy directory")
