...
ARCHIVE_ROOT_ABS="$(mkdir -p "$ARCHIVE_ROOT" && cd "$ARCHIVE_ROOT" && pwd)"

SRC_DATASET_DIR="$SRC_ROOT_ABS/datasets/$VARIANT"
SRC_TOKENIZER_DIR="$SRC_ROOT_ABS/tokenizers"
ARCHIVE_BASENAME="${VARIANT}__${TOKENIZER_BASENAME}"
ARCHIVE_PATH="$ARCHIVE_ROOT_ABS/${ARCHIVE_BASENAME}.tar.zst"
MANIFEST_PATH="$ARCHIVE_ROOT_ABS/${ARCHIVE_BASENAME}.manifest.tsv"
TOKENIZER_MODEL_PATH="$SRC_TOKENIZER_DIR/$TOKENIZER_BASENAME.model"
TOKENIZER_JSON_PATH="$SRC_TOKENIZER_DIR/$TOKENIZER_BASENAME.json"
...
if [[ -f "$TOKENIZER_MODEL_PATH" ]]; then
  TOKENIZER_PRIMARY_REL="tokenizers/$TOKENIZER_BASENAME.model"
elif [[ -f "$TOKENIZER_JSON_PATH" ]]; then
  TOKENIZER_PRIMARY_REL="tokenizers/$TOKENIZER_BASENAME.json"
else
  echo "missing tokenizer file: $TOKENIZER_MODEL_PATH or $TOKENIZER_JSON_PATH" >&2
  exit 1
fi
