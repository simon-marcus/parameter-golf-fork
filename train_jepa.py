"""
Experimental byte-level JEPA hybrid trainer for non-record parameter-golf runs.

This keeps exact BPB-compatible causal evaluation, but adds an auxiliary JEPA loss:
predict the latent embedding of the next byte patch from the current causal state.
"""
...
# HYPERPARAMETERS
# -----------------------------
# Default JEPA hybrid run:
# - byte-level `byte260` inputs
# - causal LM backbone for exact BPB scoring
# - auxiliary JEPA loss on future byte-patch embeddings
...
def load_tokenizer_luts(
    *,
    ...
    if tokenizer_kind == "byte":
        base_bytes_np = np.ones((vocab_size,), dtype=np.int16)
        has_leading_space_np = np.zeros((vocab_size,), dtype=np.bool_)
        is_boundary_token_np = np.zeros((vocab_size,), dtype=np.bool_)
        metadata = {
            "format_version": None,
            "tokenizer_kind": "byte",
            "source_model_name": "byte260",
...
def load_validation_tokens(pattern: str, seq_len: int, max_tokens: int = 0) -> Tensor:
    ...
    if max_tokens > 0:
        tokens = tokens[: min(tokens.numel(), max_tokens + 1)].contiguous()
...
def eval_val(
    ...
    acc_dtype = torch.float64 if device.type == "cuda" else torch.float32
    val_loss_sum = torch.zeros((), device=device, dtype=acc_dtype)
    val_token_count = torch.zeros((), device=device, dtype=acc_dtype)
    val_byte_count = torch.zeros((), device=device, dtype=acc_dtype)
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with autocast_ctx:
                batch_loss = model(x, y, compute_jepa=False).detach()
            batch_token_count = float(y.numel())
            val_loss_sum += batch_loss.to(acc_dtype) * batch_token_count
            val_token_count += batch_token_count
            prev_ids = x.reshape(-1)
            tgt_ids = y.reshape(-1)
            token_bytes = base_bytes_lut[tgt_ids].to(dtype=torch.int16)
            token_bytes += (has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]).to(dtype=torch.int16)
            val_byte_count += token_bytes.to(acc_dtype).sum()
...
class ByteJEPA(nn.Module):
    def __init__(
        ...
        self.jepa_in = CastedLinear(model_dim, model_dim, bias=False)
        self.jepa_out = CastedLinear(model_dim, model_dim, bias=False)
    ...
    def patch_context_states(self, hidden: Tensor) -> Tensor:
        if hidden.size(1) % self.patch_size != 0:
            raise ValueError(
                f"sequence length {hidden.size(1)} must be divisible by patch_size={self.patch_size}"
            )
        return hidden[:, self.patch_size - 1 :: self.patch_size, :]

    def predict_future_patch_latents(self, patch_states: Tensor) -> Tensor:
    ...
    def compute_jepa_loss(self, input_ids: Tensor, hidden: Tensor, target_tok_emb: nn.Embedding) -> tuple[Tensor, Tensor, Tensor]:
        patch_states = self.patch_context_states(hidden)
    ...
    def forward(
        ...
        *,
        ...
        total_loss = ce_loss
        jepa_loss = ce_loss.new_zeros(())
        var_loss = ce_loss.new_zeros(())
        pred_std_mean = ce_loss.new_zeros(())
        ...
        return total_loss, {
            ...
            "jepa_var_loss": float(var_loss.detach().item()),
            "jepa_pred_std": float(pred_std_mean.detach().item()),
        }
...
def main() -> None:
    ...
    if args.device == "auto":
        ...
        raise ValueError(f"Unsupported DEVICE={args.device!r}; expected auto, cuda, mps, or cpu")

    if distributed and device.type != "cuda":
        raise RuntimeError("Distributed training is only supported on CUDA in train_jepa.py")
    if device.type == "cuda":
    ...
    if nvidia_smi is not None:
        log0(
            subprocess.run([nvidia_smi], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False).stdout,
            console=False,
        )
    log0("=" * 100, console=False)
    ...
    log0(
        f"model_family:byte_jepa_hybrid patch_size:{args.patch_size} "
    ...
    while True:
        ...
        train_ce_loss = 0.0
        train_jepa_loss = 0.0
        train_jepa_var_loss = 0.0
        train_jepa_pred_std = 0.0
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            ...
            train_jepa_loss += metrics["jepa_loss"]
            train_jepa_var_loss += metrics["jepa_var_loss"]
            train_jepa_pred_std += metrics["jepa_pred_std"]
            (loss * grad_scale).backward()
        train_loss /= grad_accum_steps
        train_ce_loss /= grad_accum_steps
        train_jepa_loss /= grad_accum_steps
        train_jepa_var_loss /= grad_accum_steps
        train_jepa_pred_std /= grad_accum_steps

        frac = min(step / args.muon_momentum_warmup_steps, 1.0) if args.muon_momentum_warmup_steps > 0 else 1.0
        if isinstance(optimizer_matrix, Muon):
        ...
        if should_log_train:
            log0(
                ...
                f"ce_loss:{train_ce_loss:.4f} jepa_loss:{train_jepa_loss:.4f} "
                f"jepa_var:{train_jepa_var_loss:.4f} jepa_pred_std:{train_jepa_pred_std:.4f} "
    ...
    log0(
        ...
        f"{(torch.cuda.max_memory_allocated() // 1024 // 1024) if device.type == 'cuda' else 0} MiB "
        f"reserved: "
        f"{(torch.cuda.max_memory_reserved() // 1024 // 1024) if device.type == 'cuda' else 0} MiB"
    )

    if args.skip_final_eval:
        log0("skip_final_eval:enabled")
        if distributed:
