...
SUITE_ROOT="${SUITE_ROOT:-$REPO_ROOT/records/scylla_2_claude/stage2_1gpu_ttt_runs}"
...
if [[ ! -f "$BASE_CONFIG_PATH" ]]; then
  echo "missing base config: $BASE_CONFIG_PATH"
  exit 1
fi
...
ttt_cases() {
  case "$DATASET_FAMILY" in
    scylla)
      echo "a6_control a1_chunk65536 a2_chunk65536_ep2 a3_chunk65536_ep2_freeze6 a4_chunk65536_ep2_freeze6_emb1 a5_chunk96000_ep2_freeze6_emb1"
      ;;
    sp1024)
      echo "control lr0015 chunk65536 late_freeze late_freeze_emb"
      ;;
    *)
      exit 1
      ;;
...
configure_case() {
  ...
  export TTT_FREEZE_EMBEDDINGS=0
  export TTT_MOMENTUM=0.9
  export TTT_BATCH_SEQS=32
  export TTT_GRAD_CLIP=1.0

  case "$DATASET_FAMILY:$case_name" in
    scylla:a6_control)
      export TTT_LR=0.0015
      export TTT_CHUNK_TOKENS=34304
      ;;
    scylla:a1_chunk65536)
      export TTT_LR=0.0015
      export TTT_CHUNK_TOKENS=65536
      ;;
    scylla:a2_chunk65536_ep2)
      export TTT_LR=0.001
      export TTT_EPOCHS=2
      export TTT_CHUNK_TOKENS=65536
      ;;
    scylla:a3_chunk65536_ep2_freeze6)
      export TTT_LR=0.001
      export TTT_EPOCHS=2
      export TTT_CHUNK_TOKENS=65536
      export TTT_FREEZE_BLOCKS=6
      ;;
    scylla:a4_chunk65536_ep2_freeze6_emb1)
      export TTT_LR=0.001
      export TTT_EPOCHS=2
      export TTT_CHUNK_TOKENS=65536
      export TTT_FREEZE_BLOCKS=6
      export TTT_FREEZE_EMBEDDINGS=1
      ;;
    scylla:a5_chunk96000_ep2_freeze6_emb1)
      export TTT_LR=0.001
      export TTT_EPOCHS=2
      export TTT_CHUNK_TOKENS=96000
      export TTT_FREEZE_BLOCKS=6
      export TTT_FREEZE_EMBEDDINGS=1
      ;;
    sp1024:control)
...
run_case_seed() {
  ...
  export SEED="$seed"
  export RUN_ID="stage2_${DATASET_FAMILY}_${BASE_CASE}_${case_name}_seed${seed}"
...
EOF

  {
    ...
    env | grep -E '^(RUN_ID|SEED|ITERATIONS|TRAIN_BATCH_TOKENS|VAL_TOKENS_LIMIT|DATA_PATH|TOKENIZER_PATH|TOKENIZER_META_PATH|VOCAB_SIZE|QK_GAIN_INIT|XSA_LAST_N|MLP_MULT|WARMDOWN_ITERS|TTT_ENABLED|TTT_LR|TTT_EPOCHS|TTT_CHUNK_TOKENS|TTT_FREEZE_BLOCKS|TTT_FREEZE_EMBEDDINGS|TTT_MOMENTUM|TTT_BATCH_SEQS|TTT_GRAD_CLIP)=' | sort
    echo "=== command ==="
